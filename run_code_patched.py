...
class Sandbox:
    ...
    def _run(self) -> subprocess.CompletedProcess:
        ...
        """
        try:
            # self._setup_sandbox()  # V8.0: Bypassed, using Docker instead
            os.makedirs(self._tmp_dir, mode=0o755, exist_ok=True)

            network_mode = "host" if self._networking_allowed else "none"
            # V8.0 PATCH: Use docker run --runtime=runsc instead of direct gVisor
            # This works from inside a container via the Docker socket
            docker_argv = [
                "docker", "run", "--rm",
                "--runtime=runsc",
                f"--network={network_mode}",
                f"--memory={self._max_ram_bytes}" if self._max_ram_bytes else "",
                "-i",  # stdin for code input
                "cosmic-yoyo-sandbox:latest",
                "python3", "-c", self._code,
            ]
            # Filter empty args
            docker_argv = [a for a in docker_argv if a]
            started_marker_path = os.path.join(self._sandbox_shared_path, "started") if hasattr(self, '_sandbox_shared_path') else "/tmp/started"
            ...
            try:
                ...
                resource_monitor_cancel() if resource_monitor_cancel else None
