...
The current stretched phase ($G_\text{eff} < G_N$) produces a growth suppression of order 4–10% at low redshift (S₈ ≈ 0.79, consistent with the cosmic-shear DES S₈ tension), while CMB-epoch gravity was exactly Newtonian (conformal protection). The precise value is waveform-shape dependent (see [Theory](/theory/)), not a ±0.002 figure. Note the S₈ tension is bifurcated (shear-low vs cluster-high); OBT's suppression aligns with the shear side, and the eROSITA cluster-$\gamma$ link is exploratory only (audit May 2026).
...
| eROSITA DR2 | *(exploratory T3 — NOT a discriminant: f(R) is not universal-γ, γ(M) degenerate with systematics, eROSITA clusters are high-S₈; see [Theory](/theory/) audit)* | — |
