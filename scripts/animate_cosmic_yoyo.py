...
def brane_displacement(t):
    ...
    oscillation = np.where(t > 1.0, A_brane * np.sin(2 * np.pi * (t - 1.0) / T_osc), 0)
    return oscillation
...
def w_dark_energy(t):
    ...
    # Oscillates around -1
    w_lcdm = -1.0
    oscillation = np.where(t > 1.0, A_w * np.sin(2 * np.pi * (t - 1.0) / T_osc), 0)
...
ax1.set_title(
    "The Cosmic Yoyo: 13.8 Billion Years of Oscillation",
    fontsize=14,
    color="white",
    pad=15,
)
...
ax3.axhline(
    y=0.787, color="#00ffcc", linestyle="--", alpha=0.5, label="Target (Weak Lensing)"
)
...
(line2_lcdm,) = ax2.plot(
    [], [], color="gray", linestyle="--", linewidth=2, alpha=0.7, label="ΛCDM"
)
...
def animate(frame):
    ...
    if len(t_past) > 0:
        ...
        ax1.set_title(
            f"The Cosmic Yoyo: t = {t_current:.1f} Gyr (z = {np.interp(t_current, [0, 13.8], [1100, 0]):.0f})",
            fontsize=14,
            color="white",
            pad=15,
        )
...
ani = animation.FuncAnimation(
    fig, animate, init_func=init, frames=n_frames, interval=100, blit=True, repeat=True
)

# Save as GIF
writer = PillowWriter(fps=10, metadata=dict(artist="Cosmic Yoyo Theory"), bitrate=1800)
...
fig.savefig(
    "/root/bulk/oscillating-brane-DM/plots/cosmic_yoyo_frame_first.png",
    dpi=100,
    bbox_inches="tight",
    facecolor="black",
)
...
animate(n_frames - 1)
fig.savefig(
    "/root/bulk/oscillating-brane-DM/plots/cosmic_yoyo_frame_last.png",
    dpi=100,
    bbox_inches="tight",
    facecolor="black",
)
...
ani_simple = animation.FuncAnimation(
    fig_simple, animate_simple, frames=60, interval=100, blit=True, repeat=True
)

ani_simple.save(
    "/root/bulk/oscillating-brane-DM/plots/cosmic_yoyo_simple.gif",
