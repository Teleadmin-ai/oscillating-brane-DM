...
def main():
    ...
    delta_beta_scan = (alpha_em / (2 * np.pi)) * c_top * delta_phi_over_L * np.ones_like(v_scan)
    ...
    fig.suptitle(r"5D Kinematic Brane Drift — One Parameter, Two Observables"
    ...
    ax.errorbar(d_data, v_data, yerr=v_err, fmt='ko', capsize=3,
                label='Cosmicflows-4 (mock)')
    ax.axhline(y=v_bulk, color='green', linestyle=':', alpha=0.5,
    ...
    ax = axes[1]
    ax.plot(v_scan, delta_H_scan * 1000, 'r-', linewidth=2, label=r'$\delta H/H \times 10^3$')
    ...
    ax.annotate('', xy=(0.5, max(values) * 0.9), xytext=(0.5, max(values) * 0.5),
                xycoords=('axes fraction', 'data'),
                textcoords=('axes fraction', 'data'),
                arrowprops=dict(arrowstyle='<->', color='green', lw=2))
    ax.text(0.5, max(values) * 0.7, f'$v_{{bulk}} = {v_bulk:.0f}$ km/s',
