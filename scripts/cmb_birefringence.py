...
def main():
    ...
    fig.suptitle(r'CMB Birefringence — Ab Initio from 5D Geometry'
                 '\n'
    ...
    ax = axes[2]
    methods = ['Old (4D axion)\n$c_{CS}/M_{Pl}$', 'New (5D geometric)\n$\\alpha_{em} c_{top} \\phi/L$']
    ...
    # Annotate
    ax.annotate(f'$10^{{40}}$\nUNNATURAL!', xy=(0, 5.8e40),
                ha='center', va='bottom', fontsize=11, color='red', fontweight='bold')
    ax.annotate(f'$c_{{top}} = {c_top_derived:.0f}$\nNATURAL ✓', xy=(1, c_top_derived),
