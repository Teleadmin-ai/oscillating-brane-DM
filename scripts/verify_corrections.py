...
def main():
    ...
    tau_patterns = ["τ_standard = 0.0646", "τ_total = 0.0646", "within 1.5σ of Planck"]
    all_checks_passed &= check_content_in_file(
        "docs/theoretical_foundations.md", tau_patterns, "CMB optical depth correction"
    )

    all_checks_passed &= check_file_exists("plots/tau_vs_fpbh.png", "τ vs f_PBH plot")

    all_checks_passed &= check_file_exists(
        "scripts/pbh_cmb_opacity.py", "PBH CMB opacity script"
    )
    ...
    all_checks_passed &= check_file_exists(
        "scripts/analyse_posterior.py", "Posterior analysis script"
    )
    ...
    for plot in mcmc_plots:
        all_checks_passed &= check_file_exists(
            plot, f"MCMC plot {os.path.basename(plot)}"
        )

    all_checks_passed &= check_file_exists(
        "docs/posterior_table.tex", "Posterior statistics table"
    )

    # Check posterior table content
    table_patterns = ["τ_0", "f_{\\rm osc}", "68\\% CI", "\\hat{R}"]
    all_checks_passed &= check_content_in_file(
        "docs/posterior_table.tex", table_patterns, "Posterior table content"
    )
    ...
    all_checks_passed &= check_content_in_file(
        "docs/theoretical_foundations.md", ref_patterns, "Bibliography entries"
    )
    ...
    all_checks_passed &= check_file_exists(
        "scripts/plot_2d_results.py", "2D results plotting script"
    )

    all_checks_passed &= check_file_exists(
        "plots/einstein_2d_summary.png", "2D prototype summary plot"
    )

    all_checks_passed &= check_file_exists(
        "docs/einstein_2d_table.tex", "2D prototype results table"
    )
    ...
    all_checks_passed &= check_content_in_file(
        "scripts/generate_pdf.py", unicode_patterns, "Unicode cleaning implementation"
    )
