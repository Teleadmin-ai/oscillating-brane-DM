...
def main():
    ...
    print(
        f"  Second harmonic λ₂: {2.5 * lambda_1:.0f} Mpc ({2.5 * lambda_1 * 3.26:.0f} Mly)"
    )
    ...
    fig.suptitle(
        "Ultra-Large Structures — Brane Transverse Resonance\n"
    ...
    ax.axvline(x=k_ring, color="orange", linestyle="--", alpha=0.7, label=f"Big Ring")
    ax.axvline(x=k_arc, color="purple", linestyle="--", alpha=0.7, label=f"Giant Arc")
