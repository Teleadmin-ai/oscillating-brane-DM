...
class BayesianAnalyzer:
    ...
    def log_prior_osc(self, theta: np.ndarray) -> float:
        ...
        # Check bounds
        if not (self.prior_ranges_osc['tau_0'][0] <= tau_0 <= 
                self.prior_ranges_osc['tau_0'][1]):
            return -np.inf
        if not (self.prior_ranges_osc['f_osc'][0] <= f_osc <= 
                self.prior_ranges_osc['f_osc'][1]):
            return -np.inf
        if not (self.prior_ranges_osc['T'][0] <= T <= 
                self.prior_ranges_osc['T'][1]):
            return -np.inf
        if not (self.prior_ranges_osc['A_w'][0] <= A_w <= 
                self.prior_ranges_osc['A_w'][1]):
            return -np.inf
    ...
    def log_prior_lcdm(self, theta: np.ndarray) -> float:
        ...
        # Check bounds
        if not (self.prior_ranges_lcdm['H0'][0] <= H0 <= 
                self.prior_ranges_lcdm['H0'][1]):
            return -np.inf
        if not (self.prior_ranges_lcdm['Omega_m'][0] <= Omega_m <= 
    ...
    def run_mcmc(self, model: str = 'oscillating', 
                 nwalkers: int = 32, 
                 nsteps: int = 5000) -> emcee.EnsembleSampler:
        """
    ...
    def compute_evidence(self, sampler: emcee.EnsembleSampler, 
    ...
    def bayes_factor(self, log_evidence_osc: float, 
                     log_evidence_lcdm: float) -> float:
        """
