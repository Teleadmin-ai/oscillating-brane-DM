...
def generate_w_z_plot():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'w_z_oscillations.png'), dpi=150, facecolor='black')
    plt.close()
...
def generate_growth_factor_plot():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'growth_factor_comparison.png'), dpi=150, facecolor='black')
    plt.close()
...
def generate_timeline_plot():
    ...
    for i, (t, tau, epoch) in enumerate(zip(times, tensions, epochs)):
        ax.annotate(epoch, (t, tau), xytext=(10, 20), 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'cosmic_timeline.png'), dpi=150, facecolor='black')
    plt.close()
