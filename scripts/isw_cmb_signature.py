...
# Lignes des modèles
ax.plot(ell, dl_lcdm, '--', color='gray', linewidth=2.5,
        label=r'Standard Model ($\Lambda$CDM) - Overestimates power', zorder=1)
ax.plot(ell, dl_brane, '-', color='#00ffcc', linewidth=3.5,
        label=r'Oscillating Brane ($T=2$ Gyr) - Matches anomaly', zorder=2)

# Données Planck (Un point sur deux pour la clarté)
idx = np.arange(0, len(ell), 2)
ax.errorbar(ell[idx], planck_data[idx], yerr=cosmic_variance[idx],
            fmt='o', color='#ff3366', markersize=8, capsize=4,
            label='Planck 2018 Data (Unexplained deficit)', zorder=3, alpha=0.9)
...
ax.text(15, 700, 'Peak\nResonance', fontsize=10, color='yellow',
        ha='center', alpha=0.7)

# Décoration
ax.set_title("The Cosmic Yoyo Imprint on CMB via ISW Effect\n(Resolving Planck's Low-ℓ Anomaly)",
             fontsize=16, color='white', pad=20)
ax.set_xlabel("Angular Multipole ℓ (Large Cosmic Scales)", fontsize=13)
ax.set_ylabel(r"Power Spectrum $\mathcal{D}_\ell = \ell(\ell+1)C_\ell / 2\pi$ [μK²]", fontsize=13)
...
# Sauvegarder
plt.savefig('/root/bulk/oscillating-brane-DM/plots/isw_cmb_signature.png',
...
print(f"  - Physical mechanism: Oscillating w(z) → Time-varying gravitational potentials")
