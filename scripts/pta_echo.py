...
ax.axvline(2*f0, color='orange', linestyle='--', alpha=0.7, label=f'2f₀ = {2*f0:.2e} Hz')

# PTA sensitivity bands (approximate)
pta_freq = np.array([1e-9, 1e-8, 1e-7])
pta_sens = np.array([1e-15, 5e-16, 2e-15])
ax.fill_between(pta_freq, pta_sens*0.3, pta_sens*3, 
                alpha=0.3, color='gray', label='PTA sensitivity')
...
# Add annotations
ax.text(f0*1.5, h0*0.3, 'Fundamental\nmode', 
        horizontalalignment='center', fontsize=10, color='yellow')
ax.text(2*f0*1.5, h0*0.15, 'First\nharmonic', 
        horizontalalignment='center', fontsize=10, color='orange')
...
output_path_light = os.path.join(os.path.dirname(__file__), '..', 'plots', 'pta_doublet_light.png')
