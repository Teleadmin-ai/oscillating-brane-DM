...
print(f"   → Tension resolved: {abs(S8_osc - sigma8_lensing):.3f} vs {abs(S8_lcdm - sigma8_lensing):.3f}")
...
bars = ax1.bar(x_pos, s8_values, yerr=s8_errors, capsize=5,
               color=colors, alpha=0.7, edgecolor='white', linewidth=2)

# Add ΛCDM and Oscillating Brane predictions
ax1.axhline(S8_lcdm, color='gray', linestyle='--', alpha=0.8,
            label=f'ΛCDM prediction: {S8_lcdm:.3f}')
ax1.axhline(S8_osc, color='#00ffcc', linestyle='-', linewidth=3,
            label=f'Oscillating Brane: {S8_osc:.3f}')

# Shade the tension region
ax1.axhspan(sigma8_lensing - 0.01, sigma8_Planck + 0.006,
            color='red', alpha=0.1, label='S₈ Tension')
...
ax2.plot(z_array, suppression_array, color='#00ffcc', linewidth=3,
         label='Oscillating Brane')
ax2.axhline(1.0, color='gray', linestyle='--', alpha=0.8,
            label='ΛCDM (no suppression)')
ax2.axhline(0.948, color='yellow', linestyle=':', alpha=0.8,
            label=f'Average suppression: {suppression:.3f}')

ax2.fill_between(z_array, 0.94, 0.96, color='green', alpha=0.2,
...
# Save figure
plt.savefig('/root/bulk/oscillating-brane-DM/plots/s8_tension_resolution.png',
