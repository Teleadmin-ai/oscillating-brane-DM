...
- Brane tension: 7.0e19 J/m$^{2}$
- Oscillation period: T = 2.0 Gyr ($\pm$0.3)
- Extra dimension size: L = 0.2 microns
- MOND acceleration: a0 = 1.1e-10 m/s$^{2}$
...
| **S$_{8}$ suppression** | ~4--10% suppression (S$_{8}$ approximately 0.79, waveform-dependent; consistent with tension) |
...
The coming decade will be decisive. DESI and Euclid will measure the dark-energy equation of state with unprecedented precision --- the anchored stick-slip template forecasts a Decisive DeltaBIC by Year 5 --- and the theory's distinctive signature, the evolving MOND scale a$_{0}$(z) = cH(z)/2pi, will be decided by Euclid/Rubin lensing and kinematics. (The low-$\ell$ ISW remains a weak, cosmic-variance-limited ~1sigma consistency check.)
...
An independent analysis demonstrating how the Oscillating Brane Theory (Cosmic Yoyo V8.2) addresses 30 contemporary cosmological anomalies --- 2 exact quantitative resolutions with ODE integration or analytical proof (Tier 1), 14 formal analytical frameworks with closed-form derivations (Tier 2), and 14 qualitative mechanistic proposals awaiting numerical validation (Tier 3) --- within a single extra-dimensional geometric framework. *(The tier table in §8 is the register of record for every assignment --- each cell carries its audit verdict --- and all headline counts quote it.)*
...
The **Oscillating Brane Theory V8.2** proposes this paradigm shift: the Universe is a 4D membrane oscillating in a 5D Anti-de Sitter bulk ($AdS_5$), driven by a hybrid stick-slip motor coupling macroscopic Cosmic Web forcing to microscopic, thermodynamically mandated PBH dissipation (the $\ell$=0 coherence being an inflationary causal fossil; ER=EPR an optional topological interpretation).
...
### The Rotation-Curve "Diversity Problem" Is Baryonic, Not a $\Lambda$CDM Crisis (external-theory debunk)
...
### A Third Independent Confirmation of $a_0(z)$: KROSS at $z \sim 0.9$ in the MOND Regime (external-theory debunk)
...
### Planes of Satellites: The Phase-Space Coherence That $\Lambda$CDM Does Not Predict (external-theory debunk)
...
### Renzo's Rule: Baryonic Features Drive the Rotation Curve Even Where Dark Matter Dominates (external-theory debunk)
...
### Card #20 --- "Too Big To Fail" dissolved: the two-host crossfire
...
### Card #30 --- RETRACTED (June 2026): the "second axis" was the velocity circularity itself
...
### Card #31 --- Tidal dwarf galaxies: the test that assumed what its own table refutes
...
### Card #32 --- The elliptical "dearth of dark matter" is the $\mu(x)$ boost, not a challenge to modified gravity

**External theory debunked (within OBT, true if OBT true):** "the low outer velocity dispersions of ordinary elliptical galaxies (planetary-nebula kinematics --- NGC 3379/821/4494; Romanowsky et al. 2003, *'a dearth of dark matter in ordinary elliptical galaxies'*) challenge modified gravity / require fine-tuned dark haloes." **The card:** the dearth *is* the $\mu(x)$ deep-MOND boost --- the "missing mass" is the boost itself, EFE-capped and anisotropy-shaped --- and the clean, no-doubt cases are the pressure-supported **dwarfs where the orbital anisotropy is pinned or irrelevant**: the isolated deep-MOND dwarfs of card #14 ($\beta$-robust, $\sigma=(4/81\,GM_{\rm bar}a_0)^{1/4}$, $+0.04$ dex) plus **Fornax with $\beta$ PINNED by Gaia/HST proper motions** (Massari 2019, $\approx$ isotropic) under the honest Milky-Way MOND external field --- observed/predicted $\approx 1.0$ at zero tuned parameters (probe `dsph_pm`). The ellipticals are the *same mechanism* in a $\beta$-degenerate context: NGC 3379 (its companion NGC 3384 supplying a real environmental EFE) fits the full PNe+long-slit profile at $\chi^2/N \approx 1.4$ with the Cappellari MGE light, fixed $M/L$, and the literature De Lorenzi $\beta(r)$ --- nothing tuned (probe `ell_n3379`). **The why** = $\mu(x)$ reproduces pressure-supported dispersions from baryons alone; the "dearth" is the boost seen in a tracer population, not a deficit against a halo. **Honest scope:** the *elliptical* test is mass--anisotropy **degenerate** (De Lorenzi 2009: $\beta$ is free, so a declining $\sigma$ is always fittable --- OBT is a **consistent, not unique** solution; the no-doubt content lives in the $\beta$-pinned/robust dwarfs). A real **residual** survives for the faint EFE-dominated dwarfs (Draco/Ursa Minor need $\sim$2x more dynamical mass than $\mu(x)$+EFE-baryons give even isolated --- the known "MOND problem children"), so the implied $f_{\rm Weyl}(M_{\rm bar})$ is **U-shaped** --- a geometric-Weyl component needed at the faint-dwarf *and* the cluster ends (§3.28), with $\mu(x)$ sufficing in the galaxy valley --- whose amplitude is a closure/initial-condition datum (§3.4), not derived. The EFE prescription matters (the quadrature under-boosts deep-EFE dwarfs versus the proper $1/\mu(x_{\rm ext})$, by $\sim$0.2--0.3 dex). MOND-shared. *Tier: T2 (clean in the $\beta$-pinned dwarfs; the elliptical is a degenerate, consistent extension). Probes: `dsph_pm`, `ell_n3379`, `ell_pne`, `ell_jeans`.*

### Card #33 --- Stellar-stream perturbations are baryonic: the perturber tracks the orbit, no stream needs dark matter

**External theory debunked (within OBT, true if OBT true):** "stellar-stream gaps require dark-matter subhalo impacts, and the gap statistics measure the subhalo mass function" (Bonaca et al. 2019, the GD-1 gap+spur; the stream-gap subhalo-MF program). **The card:** stream perturbations are **baryonic**, and the perturber **tracks the orbit** --- disk giant molecular clouds (GMCs) and the Galactic bar for inner streams ($R_{\rm peri} < 13$ kpc, inside the molecular disk), halo globular clusters or the LMC for outer streams ($R_{\rm peri} > 13$ kpc); **no well-studied stream requires a DM subhalo**. Four computed legs: (1) the impulse kick $\Delta v = 2GM/(bw)$ is **nature-blind** --- a gap infers a perturber *mass*, not a nature (probe `stream_gaps`); (2) for Pal 5 a **restricted N-body** (Erkal--Belokurov/Bovy method, 4000 test particles, gaps forming via the $\delta s = \delta v\,t$ drift) gives GMCs $3.9 \pm 1.3$ significant gaps $\approx$ the observed $\sim 2$--5 --- the absolute count *pinned*, validated (null 0-encounter $= 0.00$ gaps; injection $1\times10^7\,M_\odot = 1.00$ gap; probe `stream_nbody`); (3) GD-1's dense Bonaca perturber is **GC-compatible by density** (MATHBLOCK1117$$G_{eff}(t) = G_N\left[1 + f_{osc}\,W\!\left(\frac{t}{T} + \frac{\delta_{bulk}}{2\pi}\right)\right]$$1118MATHBLOCKx denser than a $\Lambda$CDM subhalo of the same mass, GC-like --- the density *favors* a compact baryonic object; probe `stream_gaps`); (4) the **five-stream consilience** (probe `stream_discriminant`): Pal 5 + NGC 5466 (inner --- GMC/bar), GD-1 (GC), and Orphan + Sagittarius perturbed by the **LMC** (Erkal et al. 2019 --- a *known* baryonic object, decisive for outer streams) --- all five have an identified baryonic perturber, zero require DM. **Falsifiable forecast:** the GMC gap-contribution cuts off sharply at $R_{\rm peri} \approx 13$ kpc (DM subhalos have no such cutoff) $\to$ a future LSST test of gap-power versus pericenter. **The why** = the absent-DM-substructure theme (cards #13/#19/#20/#26/#27): with no per-object dark substructure, the perturbers are the baryonic structures each orbit actually samples. **Honest scope:** this is **not** a clean statistical *exclusion* of DM --- the gap *count* **saturates** ($\sim$3--4 for both GMCs and subhalos in the N-body, so the encounter-rate ratio does not become a count discriminant; the discriminating power lives in the perturbation amplitude/spectrum, currently data-sparse). The N-body is leading-order (secular drift; corroborated by Amorisco 2016 / Bovy 2017); GD-1 carries residual doubt (perturber mass degenerate up to $\sim 10^8\,M_\odot$, no known GC on the inferred orbit); the sample is the well-studied streams; MOND-shared (the bar/GMC/GC/LMC perturbers are standard). The debunked claim is "gaps *require* DM"; the known perturbers are baryonic. *Tier: T2 (the known stream perturbers are baryonic; not a statistical DM-exclusion). Probes: `stream_gaps`, `stream_nbody`, `stream_discriminant`.*

**Verdict.** On its home terrain --- halo-free dynamics from galaxies and ellipticals to streams and groups --- the consilience is real, strong, and computed; the OBT-distinctive lever ($a_0(z)$) is alive on three independent legs; and the closure of the paradigm requires exactly what these cards cannot reach: the cosmological derivation (dark-matter amount, growth sign, clusters), which the bulk-solver + brane-induced-AeST program (the V9.0 closure-map update below) has now **mapped** --- the FORM derived and Planck-tested (growth-sign sign + the CMB $a^{-3}$ sector), the AMOUNT the closure input.

*Closure-map update (June 2026, this work; the bulk-solver exploration in `explorations/bulk_solver/`, quarantined --- logged here for **provenance**, so nothing is lost if a better resolution later emerges). That program was pushed to a thorough map of this frontier. The **growth-sign** piece is closed within the linear-bulk chain (the warp Indicial Theorem $\to$ suppression; theory.md). The dark-matter **amount** and the **CMB background** remain closure inputs, now sharpened: the CMB acoustic-peak background needs a homogeneous $a^{-3}$ matter component (to set matter-radiation equality) that the traceless projected Weyl cannot supply (it is only $a^{-4}$ dark radiation), and no existing OBT field (Weyl, the massive radion, KK modes, string axions, the brane DBI) realises the required **halo-free** $a^{-3}$ --- so reproducing the CMB background would need an added scalar-tensor gravity sector beyond OBT's present content (the resolution is **not** derived: the warp budget closes the warped-DBI route, and the holographic/qubit route hits the same wall). OBT's dark-matter claim is therefore, honestly, the galaxy/cluster phenomenology of these cards; the cosmological amount remains a closure input. **V9.0 update (June 2026, `explorations/qubit_sensor/`, quarantined --- provenance, not promoted into the V8.2 body): the "essential ingredient" has since been identified and its FORM derived.** The added sector is a **brane-induced AeST (Aether-Scalar-Tensor) field** whose MOND free function is OBT's own geometric $\mathcal{F}_{\mathcal{Y}} = \mu(\sqrt{\mathcal{Y}}/a_0)$ (the Gauss-Codazzi $\mu(x)$), with the MOND scale $a_0(z) = cH(z)/2\pi$ tied to the aether expansion $\theta = \nabla\!\cdot\!A = 3H$ (a genuine cross-coupling, hence the distinctive $a_0(z)$), and the homogeneous $a^{-3}$ background supplied by its dust ($\mathcal{Q}$) sector. This OBT-AeST field is **Planck-consistent by the standard pipeline** --- a modified-CLASS cobaya+plik real-data MCMC gives the perturbation-level MOND amplitude $A_\mathrm{dyn} = -0.02 \pm 0.44$, consistent with $\Lambda$CDM (and the galaxy seam is acoustically stable, the dust sound speed $c_s^2 = N(x)/|\mathcal{F}_{\mathcal{QQ}}|$ with $N(x)$ OBT-derived). So the resolution's **laws are now derived** (the form, not bolted-on); what stays a closure **input** --- by design, at the epistemic level of $\Lambda$CDM's $\Omega_c$ --- is only the dark-matter **amount** (the 5:1 ratio, the dust-stiffness magnitude $|\mathcal{F}_{\mathcal{QQ}}|$). The frontier is **relocated, not erased**: the form is derived and Planck-tested *today*; the amount is the IC; the distinctive falsifiable lever stays $a_0(z)$ (Euclid). The standing intuition "an essential ingredient is missing" is thus resolved at the level of the **form**, with the **amount** an input by construction.*
...
### The Resolution of Cosmological Fine-Tuning and Dirac's Legacy
...
- The **MOND acceleration $a_0$** tracks the Gibbons-Hawking horizon scale: $a_0 = cH_0/2\pi$ (the *form* $a_0 \propto H_0$ is derived; the $O(1)$ coefficient $1/2\pi$ has prior art --- see the MOND scope note in [Theory](/theory/)).
...
### Large-Scale Structure Cosmology: The Press-Schechter $\gamma(M)$ Spectrum
...
The Oscillating Brane paradigm (Cosmic Yoyo V8.2) does not represent yet another statistical adjustment at the margins of a faltering Standard Model. It emerges as a unified extra-dimensional matrix founded on clear, interconnected mathematical deductions. Its effectiveness stems from a simple topological redefinition with a compact, honest parametric matrix --- **4 continuous EFT parameters** ($\tau_0$ and $L$ fundamental, whose cube root and size yield $\Lambda_{QCD} = 257$ MeV and $0.2\;\mu$m; $D$ and $f_{osc}$ effective macroscopic) plus one topological integer ($N = 6$) --- with the oscillation period $T = 2.000$ Gyr emerging as a derived chronodynamic eigenvalue (the $\xi R\phi$ PLL attractor).

The hybrid motor architecture --- macroscopic Cosmic Web forcing ($\mathcal{F}_{web}[E_{\mu\nu}]$) providing the muscle via Israel junction conditions, and the thermodynamically mandated micro-PBH network ($\mathcal{R}_{PBH}$) providing local dissipation as the metronome --- resolves the global phase-coherence question without any signaling: the $\ell = 0$ coherence is an inflationary causal fossil (like CMB isotropy), with ER=EPR an optional topological interpretation of the PBH entanglement.
...
The entire universe vibrates as a single entity with a derived period T = 2.000 +/- 0.003 Gyr (chronodynamic eigenvalue: 13.80/6.9, N=6 mode selected by the xiRphi PLL attractor).
...
- **xiRphi** --- Non-minimal coupling to the 4D Ricci scalar R = 6($\dot{H}$ + 2H$^{2}$). This term ensures convergence to a dynamical attractor that locks T = 2.000 Gyr despite evolving H(t) and decaying Cosmic Web forcing, resolving the chirp instability
...
**Immunity of the transverse contraction.** The $\mathcal{O}(\epsilon)$ correction to $\lambda_2$ cannot invert the contraction. The base contraction is: $\kappa = e^{-4.74} \approx 8.7 \times 10^{-3}$. The linear perturbation from cosmic expansion adds at most $|\delta\lambda_2| \sim \epsilon \times |\partial_\tau \kappa| \sim 0.14 \times \mathcal{O}(1) \sim 0.14$. Even in the worst case: $\vert\lambda_2\vert \leq \kappa + \epsilon \approx 0.009 + 0.14 = 0.15 < 1$. The inequality $\vert\lambda_2\vert < 1$ is satisfied **exactly** at the physical $\epsilon$, not merely asymptotically.

**The persistence horizon $\epsilon_0$ and safety margin.** The critical expansion rate beyond which the cycle is destroyed requires $\vert\lambda_2\vert = 1$, i.e., the transverse contraction rate $\vert\lambda_{trans}\vert = |\ln(\kappa)|/T = 4.74/2.0 = 2.37\;\text{Gyr}^{-1}$ must equal the drift speed $\epsilon$:
...
2. **Time-dependent growth suppression** --- The oscillating radion modulates the effective gravitational coupling $G_\text{eff}(t)$ over the 2 Gyr cycle, producing temporal growth suppression that resolves the S$_{8}$ tension (see below)
...
With amplitude A_w $\simeq$ 0.003, period T = 2.000 +/- 0.003 Gyr (derived chronodynamic eigenvalue), and phase $\phi_0$ = pi/2. The phase places us today at a **maximum** of w(z) approximately -0.997, with w descending into phantom territory (w < -1) in the recent past --- exactly reproducing DESI's measured phantom crossing (w_a < 0) without ghost fields.
...
**Why this resolves S$_{8}$:** The $S_8$ parameter is extracted by comparing structure growth at low redshift ($z < 1$, probed by DES/KiDS weak lensing) against the primordial prediction from the CMB ($z = 1100$, probed by Planck). During the primordial epoch, conformal symmetry ($T^\mu_\mu = 0$) froze the brane --- gravity was exactly Newtonian, and the CMB prediction $S_8 \approx 0.836$ is valid. But the late-Universe structures observed by DES grew during the **current stretched phase** of the oscillation, where $G_\text{eff} < G_N$. Structures formed more slowly than the CMB-extrapolated rate, producing a growth suppression of order 4--10% ($S_8 \approx 0.79$) --- falling within the DES Year 6 observational window ($0.790 \pm 0.018$); the precise value is waveform-shape dependent (see exact ODE integration below), so this is a consistency in the right direction, not a $\pm 0.002$ prediction.
...
The apparent DES/KiDS discrepancy is not a spatial scale effect --- it is a **temporal phase effect**: different surveys weight different redshift ranges, sampling different phases of the gravitational oscillation cycle. This *would* link the S$_{8}$ tension with the eROSITA anomaly under a single temporal mechanism --- but note (audit May 2026) the two sit on *opposite* sides of the bifurcated $S_8$ landscape (cosmic-shear-low vs cluster-high), so OBT cannot claim both; see the eROSITA audit caveat below.

### Exact ODE Integration of $D_+(a)$ and the Non-Linear eROSITA Resonance

> **️ Audit (May 2026) --- the eROSITA $\gamma$ material in this and the following two subsections is SUPERSEDED as a falsifiable claim; retained only as a Tier 3 exploratory illustration.** The measured growth index $\gamma = 1.19 \pm 0.21$ (Artis et al. 2024/2025, eRASS1) is *real* ($\sim 3\sigma$ above the GR value 0.55), but it does **not** confirm OBT: (i) it is driven by a *high* $\sigma_8$ at intermediate redshift --- *excess early growth* --- and the same survey's primary cosmology (Ghirardini et al. 2024) reports $S_8 = 0.86 \pm 0.01$, $\sigma_8 = 0.88$ (clusters *abundant*), the **opposite** of the late-time cluster *deficit* the weakened-gravity trough requires; (ii) the "constant-$G$ pipeline inflates $\gamma$ from a deficit" narrative is not how the number arose --- Artis et al. fit $\gamma$ as a free parameter directly; (iii) the claim that $f(R)$/scalar-tensor predict a *universal* $\gamma$ is **false** ($f(R)$ is scale-dependent via the scalaron Compton wavelength, hence mass-dependent; an apparent $\gamma(M)$ is moreover degenerate with mundane systematics --- AGN feedback, miscentering, non-Gaussian mass-observable scatter); (iv) the $S_8$ tension is *bifurcated* (cosmic-shear-low $\sim 0.77$ vs cluster-high $\sim 0.86$), so a single-sign temporal $G_{eff}$ aligns with shear while *worsening* the eROSITA cluster side. Deeper still, the **sign** of the growth modulation is a free bulk boundary condition (closure problem; local $\dot G/G$ forces OBT into the under-determined Weyl sector --- see the S$_{8}$ growth-sign discussion), so suppression is an *input*, not a prediction. *(Status upgraded June 2026: within the V9.0 linear-bulk model chain the sign is now derived --- suppression --- by the warp's Indicial Theorem; see the ab initio status-upgrade note after the waveform-uncertainty paragraph below.)* The Press-Schechter mechanism below illustrates how an oscillating $G_{eff}$ *would* modulate cluster abundance in a mass/redshift-dependent way; the specific $\gamma = 1.19$, the $\gamma(M)$ grid, and the "$f(R)$-exclusion / unique discriminant" claims are **withdrawn**.

**Cross-observational consistency (with honest caveats).** The growth-sector prediction ($S_8$ suppression and the eROSITA non-linear resonance $\gamma > 0.55$) introduces **no new continuous parameter fitted to the growth data**: the same $f_{osc} = 0.10$, $D = 0.90$, and the BKM-derived phase $\delta_{bulk} = 1.36$ rad are injected directly into the perturbation growth ODE. Two caveats temper the word *rigidity*, however: (i) the slip-**waveform shape** entering the ODE is a non-derivable EFT input that controls the suppression at the factor-$\sim$2 level (see below); and (ii) $f_{osc}$, $D$, $T$ are anchored to the DESI $w(z)$ amplitude and sawtooth asymmetry --- a background signal that is itself small ($A_w = 0.003$, observationally near-degenerate with $\Lambda$CDM). With these caveats, the same parameter set yields a growth suppression (order 4--10%, $S_8 \approx 0.79$) and an eROSITA-like $\gamma > 0.55$ in the same direction, on a shared parameter budget. This is a consistent semi-quantitative projection across two tensions --- not a parameter-free precision lock.
...
$$\frac{D_+(a=1, \text{OBT})}{D_+(a=1, \Lambda\text{CDM})} \approx 0.90\text{–}0.96 \quad \Longrightarrow \quad \text{suppression} \approx 4\text{–}10\%$$

$$\boxed{S_8^{OBT} \approx 0.79\text{–}0.81\;\;(\text{growth suppression}\sim 4\text{–}10\%,\ \text{waveform-shape dependent})}$$

**Waveform-shape uncertainty (honest accounting).** The waveform must be exactly zero-mean by construction (G_eff oscillates around G_N, no DC shift --- required by BBN/CMB constraints). Beyond that, the *shape* of the slip-phase discharge is a coarse-grained EFT input, **not derivable** from the base parameters: the physical radion is ultra-underdamped ($m_{rad} \sim \mathcal{O}(\text{eV})$, $\omega_0 \gg \Gamma_{slip}/2$), so only its time-averaged profile enters. *Within* a single ansatz family --- the exponential slip $f(x) = e^{-k_{slip}(x-D)/(1-D)}$, $k_{slip} \in [3,5]$ --- the variation is small ($S_8 \in [0.796, 0.798]$). **But across reasonable waveform families the result is sensitive at the factor-$\sim$2 level.** An independent recomputation of the growth ODE at the BKM-derived phase $\delta_{bulk} = 1.36$ rad, using a piecewise-linear (triangular) stick-slip discharge instead of the exponential ansatz, yields a suppression of $\sim 10\%$ ($S_8 \approx 0.75$) rather than $4.5\%$; a pure sinusoid at the same phase even flips the sign of the effect. Honest conclusion: OBT produces a growth suppression of **order 4--10%** --- robustly in the right range to address the observed $S_8$ tension ($\sim$5--10%) --- but the value cannot be quoted to $\pm 0.002$, because it depends on the non-derivable slip-waveform shape. The $\pm 0.002$ figure reflects only the variation *within* the exponential ansatz, not the genuine cross-family waveform uncertainty. The exact profile awaits the 5D NR simulation.

This growth-suppression prediction (order 4--10%) falls within the DES Year 6 ($0.790 \pm 0.018$) and KiDS-1000 ($0.759 \pm 0.024$) observational bounds, **consistent with** the $S_8$ tension. We emphasize this as a consistency in the right direction and magnitude, not a precision determination: the suppression amplitude (and, for some waveform conventions, even its sign) depends on the non-derivable slip-waveform shape and on the BKM phase, so no $\pm 0.002$-level claim is warranted.

**Ab initio status upgrade --- the V9.0 gate program and the Indicial Theorem (June 2026).** The growth-sector *sign* --- flagged throughout the May-2026 audits as a free bulk boundary condition (an input at the epistemic level of $\Lambda$CDM's $\Omega_c$) --- has since been **derived within a V9.0 model chain** (the characteristic bulk-solver gate program, `explorations/bulk_solver`, Gates 0--9; quarantined V9.0 work, summarized here because it bears directly on this section's epistemic status). The chain: (i) a validated double-null AdS$_5$ solver (CHKS-class, MMS-validated at order 2, GR recovered at 2.4%) shows the bulk-wave sector --- conservative, dissipative, compact, or Goldberger-Wise-stabilized --- supplies *neither* the $S_8$ phase *nor* a growth sign (all driven channels die as $\sim k^2$--$k^4$ at cosmological scales); (ii) the PBH "tent-peg" microphysics derives the network spring ($k_{peg} = 4\pi\sigma r_h$, exact perforated-brane geometry), the release threshold $d_{crit} = r_h$ --- hence $\phi_{crit} \sim 0.1L$ from the EMF heavy tail --- and the stick-slip **sawtooth emerges** untuned from the EMF-distributed thresholds (slip duty 0.13 vs the posited 0.10); (iii) the **Indicial Theorem**: near the brane the bulk master equation carries the AdS warp term $-1/(4z^2)$, a regular-singular point with *degenerate indicial exponents* $(\tfrac{1}{2},\tfrac{1}{2})$ --- every bulk configuration therefore projects onto the brane with logarithmic slope $s = d\ln\Omega/d\ln z_b \in (-2,-1]$, and the physical modulation coefficient (CHKS dictionary, Eq. 30 of arXiv:0705.1685; the potential route and the Weyl-density route give the *same* answer) is $c_{phys} = s + 2 \in (0,1]$, **strictly positive**: the sign of the radion--dark-matter coupling is forced by the warp's indicial structure. At the physical hierarchy ($k\ell \sim 10^{-26}$) the logarithmic corrections collapse and $c_{phys} = 0.983 \pm 0.02$; the recovered amplitude $f_{eff} = c_{phys} \times (\phi_{crit}/L) \times (5/6) \approx 0.08$ independently lands on the posited $f_{osc} = 0.10$ within $\sim$20%. Integrating the growth equation with the derived sawtooth, the derived coupling, and the chronological anchor (phase 0.9 today) yields $\Delta S_8/S_8 \approx -3.5\%$ to $-5.6\%$ --- **suppression, derived**, at the low end of the 4--10% band above. A sign flip would require bulk structure at sub-$L$ transverse scales ($k z_b \sim 1$), twenty-six orders of magnitude from any astrophysical configuration. **Named premises (the honest perimeter):** linear-bulk treatment of the Weyl configuration (exact at halo densities, where $\rho/\sigma$ is minuscule), quasi-static profile premise, leading sub-horizon dictionary, EdS growth background; the dark-matter *abundance* remains initial-condition data --- the chain derives the *coupling*, not the amount. Within these premises the sign ceases to be an input: *suppression is the model-chain prediction*, and an observed $S_8$ **enhancement** would refute the chain.
...
The measured $\gamma = 1.19$ is not an anomalous departure from General Relativity. It is the expected output of applying a constant-$G$ pipeline to an oscillating-$G$ universe, amplified by the exponential sensitivity of the Press-Schechter mass function. The same mechanism, with the same BKM-derived phase, produces both a growth suppression ($S_8 \approx 0.79$, linear) and an apparent $\gamma > 0.55$ (non-linear, eROSITA), in the same direction (precise magnitudes waveform-shape dependent).
...
**Exploratory expectation (NOT a strict falsifiable discriminant --- see audit caveat above):** the mechanism *qualitatively* suggests a mass-dependent apparent index ($d\gamma_{app}/dM > 0$), since the Press-Schechter sensitivity grows with peak height $\nu$. But this is **not** a unique OBT signature: (a) $f(R)$ gravity does **not** predict a universal $\gamma$ --- its scalaron Compton wavelength makes growth scale-dependent, hence mass-dependent (only DGP gives a near-universal $\gamma \approx 0.68$); and (b) a mass-dependent apparent $\gamma$ is degenerate with ordinary cluster-cosmology systematics (mass-dependent AGN feedback, miscentering, non-Gaussian mass-observable scatter). The specific grid below is therefore illustrative of the *mechanism*, not a falsification test. Breaking the degeneracy would require clean cross-probes (RSD $f\sigma_8$, void profiles, kSZ), not cluster counts alone.

**Epistemological status (Tier 3 exploratory, demoted May 2026).** Following the audit above, the $\gamma = 1.19$ "derivation" is demoted from a falsifiable Tier 2 prediction to a Tier 3 *exploratory illustration*: it relies on the Tinker multiplicity function (an empirical fit calibrated against $\Lambda$CDM N-body, applied here outside its calibration to an oscillating $G_{eff}$ background), the growth *sign* it assumes is a free bulk boundary condition rather than a derived output (status upgraded June 2026: now derived --- suppression --- within the V9.0 linear-bulk chain; see the Indicial Theorem note above), and the target number is anyway not what eROSITA's primary cosmology (high $S_8$, cluster-abundant) supports. A first-principles treatment would require both a resolved bulk boundary condition (to fix the sign) and full 5D-coupled N-body simulations (to fix the magnitude).

### The Falsifiable $\gamma(M)$ Spectrum: Continuous Amplification Prediction for eROSITA DR2

*(This subsection is superseded --- see the eROSITA audit caveat above. Retained as a Tier 3 mechanistic illustration; the $\gamma(M)$ grid is not a falsifiable discriminant.)* The measurement of an anomalous growth index $\gamma \approx 1.19$ by the eROSITA X-ray satellite (Artis et al. 2024/2025; GR predicts $0.55$) is a real $\sim 3\sigma$ result, but as the audit explains it is driven by high intermediate-$z$ $\sigma_8$ (cluster-abundant), not the cluster deficit OBT would produce. In OBT V8.2 the oscillating-$G_{eff}$ mechanism below *would* modulate cluster abundance, but the sign is a free bulk boundary condition, not a derived mirage. The time-dependent gravitational coupling $G_{eff}(t)$ elevates the spherical collapse threshold $\delta_c(z)$, which exponentially suppresses the abundance of massive clusters. A constant-$G$ inference algorithm compensates for this deficit by artificially inflating the extracted growth index $\gamma$.
...
Injecting this analytically derived phase lag into the BDF stiff ODE integration (without optimization) yields a growth suppression of order 4--10% ($S_8 \approx 0.79$): the $\sim 4.5\%$ figure corresponds to the exponential-slip ansatz, but an independent recomputation with a triangular slip gives $\sim 10\%$ at the same derived phase (a pure sinusoid even flips the sign). The value is therefore waveform-shape dependent --- not $\pm 0.002$. It falls within the DES Year 6 ($0.790 \pm 0.018$) and KiDS-1000 ($0.759 \pm 0.024$) bounds, consistent with the tension.
...
**Epistemological status.** The tensor-scalar phase delay $\delta_{bulk}$ is **not** a fitted parameter: it is analytically derived from the base EFT parameters via the BKM arctan formula, and is structurally insensitive to the choice of $z_{eff}$ (the arctan saturation above). *This part is robust.* What is **not** pinned to $\pm 0.002$ is the resulting $S_8$ value: an independent recomputation shows the growth suppression depends at the factor-$\sim$2 level on the non-derivable slip-waveform shape (exponential ansatz $\sim$4.5%, triangular $\sim$10%, sinusoid even sign-flipped at the same derived phase). The honest statement is that the same $G_{eff}(t)$ machinery, with the BKM-derived phase, produces a growth suppression of order 4--10% (addressing the $S_8$ tension in the right direction) and an apparent $\gamma > 0.55$ (eROSITA-like, via non-linear Press-Schechter amplification) --- a consistent semi-quantitative projection on a shared parameter budget, not a parameter-free precision prediction.
...
**The genuinely novel contributions of OBT V8.2** are: (1) the seamless *embedding* of this established thermodynamic boundary acceleration into the exact 5D Shiromizu-Maeda-Sasaki (SMS) braneworld geometry, which enables: (2) the *rigorous geometric derivation* of $\mu(x) = x/\sqrt{1+x^2}$ as the exact trigonometric projection of the 5D extrinsic curvature via the **Gauss-Codazzi orthogonality theorem** (not an empirical fit or ad-hoc postulate --- see Section 3 below); and (3) the *geometric reinterpretation of the cluster dark matter* as the mass-driven projected Weyl component $\mathcal{E}_{\mu\nu}$ (a real dark-matter component absent from MOND) which --- together with MOND's classic *insufficiency* at cluster scales --- accounts for why MOND succeeds for galaxies yet fails for clusters. *(Audit June 2026: the earlier framing of (3) as an "exact sinc extinction mechanism" is **corrected** --- see the Dynamic Averaging section below. The orbital averaging gives at most a mild saturating suppression $\mathcal{W}\sim0.7$, not extinction; the cluster failure is MOND's insufficiency carried by the mass-driven Weyl, whose amplitude is a closure/initial-condition datum --- a geometric reinterpretation of the cluster dark matter, not a derived sinc extinction.)*
...
**Epistemological note --- the closure problem (what is reinterpreted vs. what is derived).** OBT reinterprets the *nature* of dark matter (geometric, sourced by $\mathcal{E}_{\mu\nu}$, not particulate), but it does **not derive its abundance**. Two honest limits must be stated explicitly. (i) **Amplitude.** $\mathcal{E}_{\mu\nu}$ is a free bulk field, constrained only by the Bianchi identity $\nabla^\mu\mathcal{E}_{\mu\nu} = \kappa_5^4\nabla^\mu\pi_{\mu\nu}$ --- it is **not** sourced by the $\sim 1\%$ PBH rest mass, so there is no factor-$\sim 500$ amplitude contradiction, but equally the observed $\sim 5{:}1$ dark-matter-to-baryon ratio is a **bulk boundary condition**, not a first-principles output. (ii) **Clustering and the dark-radiation tension.** The *homogeneous* part of $\mathcal{E}_{00}$ behaves as "dark radiation" diluting as $a^{-4}$, constrained by BBN ($\Delta N_{\rm eff}$) to $\lesssim 10^{-5}$ of the dark-matter density; the gravitating dark matter must therefore reside **entirely in the inhomogeneous component** --- a configuration with cosmic mean $\approx 0$ yet locally $\sim 5\times$ the baryon density around every structure (with compensating repulsive $\mathcal{E}_{00}$ in voids, since $\mathcal{E}^\mu_\mu = 0$). Whether such a configuration, with $a^{-3}$-like (CDM-mimicking) growth, actually emerges is **under-determined by the braneworld closure problem** (Koyama; Maartens 2004): the brane perturbation equations do not close without the full 5D bulk solution. Epistemic status: a *consistent geometric reinterpretation* of dark matter --- at the same level as $\Lambda$CDM's fitted $\Omega_c$ --- and a conjecture pending a dynamical bulk solution, **not** a derivation of the dark-matter abundance or its clustering. The kinematic results below (MOND $\mu(x)$, the sinc filter, the Bullet offset *given* a collisionless dominant component) are robust; the *amount and growth* of that component are inputs.
...
**Quantitative validation (SPARC catalog, 135 galaxies):** The zero-free-parameter prediction ($a_0 = cH_0/2\pi$, $\mu(x) = x/\sqrt{1+x^2}$) was tested against the SPARC galaxy rotation curve catalog (Lelli, McGaugh & Schombert 2016). The emergent MOND formalism reproduces observed flat rotation velocities with an RMS scatter of **29.3 km/s** ($\sigma = 0.0854$ dex). For comparison, the standard NFW dark matter halo profile --- requiring **2 free parameters per galaxy** (concentration $c$ and virial mass $M_{200}$) --- achieves a worse fit with RMS = **35.0 km/s** ($\sigma = 0.101$ dex). A zero-parameter geometric prediction outperforming a 270-parameter fit constitutes powerful evidence for the emergent nature of galactic dark matter dynamics. *(Clarification: "zero free parameters" means zero **dark-sector** parameters --- no per-galaxy halo. Stellar mass-to-light ratios are fixed by a 3.6 mum stellar-population prior, and distances/inclinations come from the data with their own uncertainties, not fitted to the curve. This is the standard MOND/RAR convention of McGaugh-Lelli-Schombert; the comparison "0 vs 270" is dark-sector parameters.)*

### Dynamic Averaging: The MOND Cluster Insufficiency and the Mass-Driven Weyl Component

**️ Reviewer audit correction (June 2026) --- read before this section.** The central claim of this section as originally written --- that the MOND boost is *extinguished* at clusters by the orbital-averaging sinc reaching an "exact zero" ($\mathcal{W}_{exact}(1)\equiv0$, the "topological protection theorem"), so that the cluster missing mass is the sinc-resurrected Weyl fluid --- has been **corrected** on three independent grounds. **(1) The "exact zero" is a signed-$a_0$ / Jensen artifact.** This section averages the *signed* transverse acceleration $\langle a_0\rangle$ (Eq. below), which vanishes at $t_{dyn}=T$. But the MOND boost depends on $a_0^2$ (the Gauss-Codazzi quadrature $g_{5D}=\sqrt{g^2+a_0^2}$ derived in the previous section) --- a positive-definite quantity. The tracer averages the *boost* (a convex function of $a_0$), not $a_0$; by Jensen $\langle\text{boost}(a_0)\rangle\neq\text{boost}(\langle a_0\rangle)$. Using this section's own waveform $a_0(t)=a_0^{max}\sin(2\pi t/T)$, the physical averages are $\langle a_0^2\rangle = a_0^{max\,2}/2$ ($\Rightarrow a_{0,eff}=\sqrt{\langle a_0^2\rangle}=0.71\,a_0^{max}$) and $\langle\vert a_0\vert\rangle = 0.64\,a_0^{max}$ --- **the boost saturates at $\mathcal{W}\sim0.6$--$0.7$, it does not extinguish.** The "$\mathrm{sinc}(\pi)=0$, topologically protected" result averages the one quantity (signed $a_0$) that the physical boost does *not* depend on. **(2) The cluster failure is MOND's classic *insufficiency*, not extinction.** A this-work X-COP RAR (12 clusters, M_FORW hydro + FGAS, exact RAR at *full* boost) gives $M_{hydro}/M_{MOND\text{-}full}=1.30$ (median) at $r\sim2$ Mpc --- MOND under-predicts by $\sim$30% --- and in the deep-Newtonian core ($g\gg a_0$, $\mu\to1$) MOND gives no boost at all while the Weyl supplies $5$--$8\times$; *nowhere* is the boost sinc-extinguished. The cluster dark matter is the **mass-driven geometric Weyl** (insufficiency-carried; amplitude a closure/initial-condition datum, derivable form --- see the closure note in the previous section and Discoveries §3.4/§3.28). **(3) The empirical "band" evidence was a velocity-circularity artifact.** The SPARC outermost-point deficit-vs-$T_\kappa$ trend ($-0.068$ dex; Discoveries cards #29/#30) is a tautology --- the RAR residual carries $+2\log V_{obs}$ and $T_\kappa$ carries $-\log V_{obs}$; the de-circularized test (residual vs the baryon-predicted period, no $V_{obs}$) is null ($r=+0.005$, $p=0.96$, $N=131$). Cards #29/#30 are **retracted**. The mild suppression ($\mathcal{W}\sim0.7$) remains a *prediction* of the $a_0^2$ averaging, consistent with MW/M31 satellite kinematics (which also tolerate full MOND) --- **currently unconfirmed**, not a detection. **What survives intact:** the adiabatic side (galaxies at full MOND, $\mathcal{W}\approx1$ --- data-mandated, §below); the geometric Weyl as the cluster dark matter (the *amount* a closure input); and the Bullet Cluster ($\mathrm{sinc}(\pi t_{cross}/T)\approx1$, the fast non-virialized regime). *The derivation below is retained for the record as originally written; read "extinction / sinc$(\pi)=0$ / 0% survival" throughout as the corrected "mild saturating suppression $\mathcal{W}\sim0.7$, with the mass-driven Weyl carrying the cluster dark matter."*
...
- For $t_{dyn} = T$ (exact resonance): the *signed* $\langle a_0\rangle$ time-average vanishes ($\text{sinc}(\pi) = 0$). *[Audit June 2026: this is the signed-$a_0$ artifact --- the physical boost depends on $a_0^2$ ($\langle a_0^2\rangle = a_0^{max\,2}/2 \neq 0$) and saturates at $\mathcal{W}\sim0.7$; the "MOND correction self-destructs" reading is corrected to "mild suppression, Weyl-carried insufficiency" per the section preamble.]*
...
The topological identity $\sin(n\pi) = 0$ guarantees that the averaging kernel vanishes exactly at $t_{dyn} = T$ for any waveform shape. This mathematical zero does not depend on the amplitudes $A_n$, phases $\varphi_n$, or duty cycle $D$. **[Audit June 2026: this zero is the signed-$a_0$ / Jensen artifact described in the section preamble. It averages the *signed* $\langle a_0\rangle$, but the physical MOND boost depends on $a_0^2$ (the Gauss-Codazzi quadrature), whose orbital average $\langle a_0^2\rangle = a_0^{max\,2}/2$ is **non-zero**. The averaging kernel for the boost saturates at $\mathcal{W}\sim0.7$; it does not reach zero. The "topological protection" protects the one quantity the boost does not depend on.]**
...
The MOND phenomenology operates at **full power** ($> 98\%$) for all galaxies ($t_{dyn} < 350$ Myr) --- the adiabatic regime, data-mandated (§ARA below). At cluster scales the *signed*-$a_0$ sinc reaches a formal zero in the table above; **[audit June 2026: the physical boost (depending on $a_0^2$) does not --- it saturates at $\mathcal{W}\sim0.7$. The cluster "MOND failure" is not extinction but the classic *insufficiency* (full MOND under-predicts cluster mass by $\sim$30% at $r_{500}$ and gives no boost in the deep-Newtonian core), with the mass-driven Weyl carrying the dark matter (closure/IC amplitude). Read the cluster row of the table as "$\mathcal{W}\sim0.7$, insufficiency carried by the Weyl", not "0%".]**
...
- **Radial (resolved-profile) form:** for systems analyzed radially, the local averaging window is the **orbital period** $t_{dyn}(r) = 2\pi r/V_c(r)$ --- the form used in the X-COP cluster anatomy (Discoveries §3.28), where it spans 0.25 Gyr (BCG core, $\mathcal{W} \approx 0.96$: MOND *survives in cluster cores*) to 5.6 Gyr ($r_{500}$), and is empirically validated by the hierarchical fit ($\chi^2/N = 1.04$). The two forms agree on every verdict where they overlap: the global form is the $\sigma$-weighted outer-region average of the radial form, and both apply at most a *mild* suppression ($\mathcal{W}\sim0.7$, audit June 2026 --- not the "extinction" of the original wording) to the relaxed-cluster bulk boost, with the mass-driven Weyl carrying the cluster dark matter.
...
**The convention-sensitive zone (honest flag).** For **diffuse pressure-supported systems** --- ultra-diffuse galaxies of the NGC 1052-DF2/DF4 class, the "feeble giants" (Crater II, Antlia 2), and compact groups --- the choice of radius ($r_{half}$ vs outer extent) and form ($R/\sigma_v$ vs $2\pi r/V_c$) moves $t_{dyn}$ by factors of a few **inside the steep part of the filter**: e.g. for Crater II the bracket spans $t_{dyn} \approx 0.4$--$1.4$ Gyr, i.e. $\mathcal{W} \approx 0.94$ down to $\approx 0.35$. *This zone is now resolved by the tracer-weighted ARA refinement immediately below*: DF2/DF4-class systems come out adiabatic-dominated ($\mathcal{W}_{eff} \approx 0.83$, bracket 0.70--1, a $\leq 5$--9% effect on $\sigma$) and are **de-gated**; the residual $\mathcal{O}(1)$ uncertainty applies only to tracers inside the resonance band $T_{int} \in [T/2, 2T]$. Outside the zone the conventions are immaterial: all classical compact dwarfs have $t_{dyn} \leq 0.4$ Gyr under *every* convention ($\mathcal{W} \geq 0.94$, a $\leq 1.6\%$ effect on $\sigma$ --- the dwarf-dynamics results are convention-safe), and clusters are extinguished under every convention.

**The tracer-weighted window: the Adiabatic-Resonant-Averaging (ARA) refinement (June 2026).** The question left open by the pinned convention --- *which* window applies to a pressure-supported tracer population --- is resolved by treating each tracer as a **driven oscillator** under the brane's amplitude modulation, which is standard perturbation theory in its two limits:

1. **Per-tracer response function.** The oscillating tilt is an amplitude modulation of the effective gravity at drive frequency $\omega_d = 2\pi/T$. A tracer with internal frequency $\omega_{int}$ --- the epicyclic frequency $\kappa = \sqrt{2}\,\Omega$ for near-circular orbits (internal period $T_\kappa = T_{orb}/\sqrt{2}$ on a flat curve), the radial frequency $2\pi/T_r$ for pressure-system orbits --- responds **adiabatically** when $\omega_d < \omega_{int}$: the orbit tracks the *instantaneous* $a_0(t)$, and since the galactic anchor $a_0 = cH_0/2\pi$ is calibrated on exactly such adiabatic systems, the response is $F = 1$ identically (**no suppression --- averaging does not apply in the adiabatic regime**). When $\omega_d > \omega_{int}$, the slow orbit cannot follow and averages the fast modulation over its own period --- V8.2's boxcar --- giving the $\vert\mathrm{sinc}(\pi T_{int}/T)\vert$ envelope. The crossover sits at $T_{int} = T$; the band $T_{int} \in [T/2,\,2T]$ contains the $\kappa$-resonance, where the linear response is singular and the $\vert\mathrm{sinc}\vert$ envelope is retained as the EFT interpolation with an $\mathcal{O}(1)$ flag. The **system filter** is the light-weighted average $\mathcal{W}_{eff} = \langle F(T_{int})\rangle_{tracers}$.

2. **In-corpus empirical discrimination (the adiabatic side is data-mandated).** Across the SPARC catalog (175 galaxies, `scripts/sinc_tracer_window.py`), the outermost-point epicyclic periods have median $T_\kappa = 0.54$ Gyr; 174/175 galaxies are fully sub-crossover and only one (UGC 9133, $T_\kappa \approx 2.05$ Gyr; next: NGC 289 and UGC 128 at 1.87 Gyr) reaches the resonance. ARA therefore predicts $F = 1$ catalog-wide --- exactly what the RAR demands. A raw per-orbit boxcar (no adiabatic regime) would instead impose 36--97% $a_0$ suppression on the outermost points of the 20 band-straddling galaxies --- **excluded** by the declining-curve RAR residuals ($+0.000 \pm 0.089$ dex; Discoveries §3.8). The X-COP cluster anatomy is preserved on both ends: BCG cores are adiabatic ($F \approx 1$, the observed core MOND survival) and the relaxed-cluster bulk has $T_r \approx 3$--6 Gyr $\gg T$ (deep averaging $\to$ mild suppression $\mathcal{W}\sim0.7$, the mass-driven Weyl carrying the cluster dark matter; audit June 2026 corrected the earlier "extinction"; Discoveries §3.28).

3. **Tracer-weighted windows for the diffuse pressure systems** (isotropic Plummer Monte-Carlo normalized to the observed $\sigma_{los}$, $R_e$; light-weighted radial-period distributions):

| System | med $T_r$ (Gyr) | light frac. $T_r > T$ | $\mathcal{W}_{ARA}$ | band-edge bracket | $\sigma$ suppression ($\propto \mathcal{W}^{1/4}$) |
|:---|:---:|:---:|:---:|:---:|:---:|
| NGC 1052-DF2 | 0.83 | 0.19 | **0.83** | 0.70--1 | $\leq 5$% (bracket $\leq 9$%) |
| NGC 1052-DF4 | 1.22 | 0.29 | **0.74** | 0.54--1 | $\leq 7$% |
| Crater II | 1.27 | 0.30 | **0.73** | 0.52--1 | $\leq 8$% |
| Antlia 2 | 1.55 | 0.38 | **0.66** | 0.41--1 | $\leq 10$% (+ non-equilibrium caveat) |

4. **Consequences.** (i) **DF2/DF4-class UDGs are de-gated**: they are adiabatic-dominated, so the full external-field machinery applies with a bracketed $\leq 9$% $\mathcal{W}$-correction on $\sigma$ --- OBT's prediction there is essentially the EFE-MOND prediction (not OBT-distinctive, a consistency arena). (ii) The feeble giants carry modest central corrections ($-8$% Crater II) inside their stated brackets --- subdominant to the tidal effects that dominate those systems. (iii) **A new future falsifiable emerges**: rotation-supported tracers beyond the epicyclic resonance ($T_{orb} > \sqrt{2}\,T \approx 2.8$ Gyr, i.e. ultra-extended HI discs at $r \gtrsim 70$--90 kpc) must show a *declining* MOND boost where constant-$a_0$ MOND predicts none --- UGC 9133, NGC 289 and UGC 128 are the first candidates. **Channel note --- the photon channel is instantaneous (pinned June 2026).** Under ARA the suppression lives in the *tracer's orbital response*, not in the field: the tilt geometry oscillates at full amplitude, and a probe with no orbital period samples it instantaneously. Photons (crossing times $\sim$ Myr $\ll T$) therefore see the **full boost at all radii** --- the lensing channel carries $\mathcal{W} \equiv 1$. This is the basis of the lensing-vs-dynamics discriminant (the "scissor"): at radii where bound tracers are deep post-band, lensing keeps the full RAR while orbital dynamics lose it --- a signature neither constant-MOND (both full) nor $\Lambda$CDM (both halo-like) can produce. First blade: the KiDS-isolated lensing RAR at $r_{eff} \sim 100$--450 kpc sits at/above the full RAR ($+0.13$ dex, mostly the known two-halo term). **[Audit June 2026: the strong "$-0.3$ to $-1$ dex orbital-channel suppression" once predicted here is the signed-$a_0$ extinction --- it is *refuted* by MW/M31 satellite kinematics, which show the orbital boost saturating near full MOND ($\mathcal{W}\sim0.7$, M31 consistent with 1), not collapsing. The corrected scissor is at most a *moderate* effect ($\lesssim0.1$--0.15 dex), is degenerate with galaxy size, and is currently *unconfirmed*; the clean test is lensing vs satellite-kinematics matched in host size. The scissor is a candidate consistency check, not an established distinctive signature.]** *Consistency with the Bullet Cluster section:* the $\mathrm{sinc}(\pi t_{cross}/T)$ factor there modulates the **dynamical** response of the colliding system (the infall velocity --- massive tracers), where it evaluates to $0.995 \approx 1$; under the pinned convention the lensing term itself carries $\mathcal{W} = 1$ exactly --- numerically indistinguishable ($0.995$ vs $1$), conceptually now unambiguous.

**Derivation status (honest):** the adiabatic and averaging limits are derived (standard perturbation theory) and the adiabatic side is independently data-mandated; the resonance-band interpolation remains an EFT envelope --- now **data-bounded**: a SPARC outermost-point stack of RAR residuals versus $T_\kappa$ (`scripts/sinc_tracer_window.py`, band-entry section) gives $\mathcal{W}_{band} > 0.61$ at 95% over $T_\kappa = 1$--2.2 Gyr, with a candidate monotonic entry trend ($-0.068 \pm 0.020$ dex at $T_\kappa > 1$ Gyr, 3.4$\sigma$) that is *degenerate with the external-field effect*; the Plummer/isotropy modeling carries $\pm 10$--20% on $T_r$, folded into the brackets above. **[Audit June 2026 --- the band trend is a velocity-circularity ARTIFACT, not evidence.** The RAR residual carries $+2\log V_{obs}$ and $T_\kappa$ carries $-\log V_{obs}$, so they are anti-correlated by construction; the de-circularized test (residual vs the baryon-predicted period $T_{\kappa,pred}$, no $V_{obs}$) is **null** ($r=+0.005$, $p=0.96$, $N=131$). The cards that rested on this trend (Discoveries #29 Malin 1, #30 second-axis) are **retracted**. The mild suppression ($\mathcal{W}\sim0.7$) is the *prediction* of the $a_0^2$ orbital averaging; it is consistent with MW/M31 satellite kinematics (which also tolerate full MOND) but is **currently unconfirmed** --- the SPARC "band" was no evidence. Only the adiabatic side (galaxies $\mathcal{W}\approx1$) is empirically established here.]**
...
**4. The mass-driven Weyl component: the dark matter of clusters.** The cluster dark matter is the projected Weyl term $\mathcal{E}_{00}$, **carrying MOND's classic insufficiency** (full MOND under-predicts cluster mass by the factor $2$--$8$; the mild orbital suppression $\mathcal{W}\sim0.7$ only deepens it slightly --- *not* the "$\langle a_0\rangle\to0$ extinction" of the original wording, corrected in the audit preamble). The effective gravitational acceleration for an $N$-body cluster system is:
...
- **Not MOND alone** (the boost --- full, or mildly suppressed to $\mathcal{W}\sim0.7$ --- is *insufficient* at cluster scales: the classic factor 2--8 problem)
...
- **The background acceleration** $a_0 = cH_0/(2\pi) \approx 1.1 \times 10^{-10}$ m/s$^{2}$: derived from Gibbons-Hawking thermodynamics of the cosmic horizon via the Unruh effect. A macroscopic thermodynamic constant, not an empirical fitting parameter.
...
The effective interpolation function is $\mu_{eff} = 0.98 \times \mu(x)$. Because $v_{circ} \propto \mu^{-1/2}$, a 2% suppression in $\mu$ translates to a $\sim 1\%$ shift in the predicted velocity ($v \to 1.01\,v$). This minuscule kinematic shift is completely absorbed by the standard 5%--10% observational uncertainties in galaxy distance and inclination angle. The mechanics of galaxies reside exclusively in the quasi-static geometric regime. The 5D tilt survives completely at galactic scales; at cluster scales it is at most *mildly* suppressed ($\mathcal{W}\sim0.7$ --- audit June 2026, not the "self-destruction" of the original wording), with the mass-driven Weyl component carrying the cluster dark matter (see the Dynamic Averaging audit preamble).
...
**Multi-scale check: wide binaries (Chae 2025) --- CONTESTED.** The identical $\mu(x)$ formula, evaluated at Gaia DR3 wide-binary scales, predicts $\gamma_g(u) = [(1+\sqrt{1+4/u^2})/2]^{1/2}$, yielding $\gamma_g \in [1.58, 1.80]$ over the Chae low-acceleration bin --- in $<1\sigma$ agreement with Chae's $\gamma_g = 1.48^{+0.33}_{-0.23}$. **Caveat (audit May 2026):** the wide-binary anomaly itself is *disputed* --- Pittordis-Sutherland (2023) and Banik et al. find Newtonian models fit better, with the disagreement on hidden-triple contamination unresolved. And $\gamma_g(u)$ is MOND's boost (OBT inherits it), so even if confirmed it tests MOND-vs-Newton, not OBT-vs-MOND. This is therefore consistency with one contested analysis, not a decisive multi-scale validation. See [Discoveries §3.6](/discoveries/).

## Stability

### The Adiabatic Shield

The brane oscillation frequency is $\nu$ ~ 1.6 x 10$^{-17}$ Hz (period 2 Gyr), while the lightest Kaluza-Klein excitations have mass ~3.78 eV (flat-space; ~1.87 eV warped), corresponding to $\nu$_KK ~ 9.1 x $10^{14}$ Hz. The ratio is:
...
In a 1D spatial grid, mechanical energy lacks transverse dimensions to dissipate into; it reflects and accumulates destructively. However, in the physical (3+1)+1D topology, the highly accelerated motion of the 3-brane during the violent non-linear "slip" phase (phï >> 0) makes it a macroscopic source of gravitational radiation. According to 5D General Relativity, an accelerating massive brane emits transverse-traceless bulk gravitons (Kaluza-Klein modes) into the extra dimension. This continuous emission of Dark Radiation introduces a highly non-linear radiation reaction force ($\Gamma$_rad) into the radion dynamics.

During the slow stick phase, acceleration is minimal and $\Gamma$_rad approximately 0. But the moment the brane slips and accelerates, $\Gamma$_rad spikes, instantly capping the maximum velocity and evacuating excess geometric strain into the AdS bulk. This fundamental thermodynamic mechanism guarantees that the macroscopic observable w(z) remains in the stable perturbative regime (A_w ~ O(10$^{-3}$)), definitively resolving the 1D runaway artifact using the pure laws of 5D gravity.
...
- **Classical dissipation** (Bremsstrahlung): energy escapes as propagating waves $\to$ requires $\omega > m_n$ (kinematically blocked by $e^{-3.6 \times 10^{31}}$)
- **Quantum dissipation** (informational viscosity): energy absorbed locally by PBH horizons $\to$ requires only $S_{BH} \gg m_1 T_{slip}$ (always satisfied: $10^{56} \gg 10^{31}$)

*Theorem (Kinematic Blockade and PBH Necessity).* Let $\mathcal{M}$ be a smooth Nambu-Goto 3-brane oscillating in AdS$_5$ with frequency $\omega \ll m_1 = j_{1,1}/L$. Then $P_{KK}[\mathcal{M}] = 0$ and the brane motion is undamped. A stable cosmological attractor ($\Gamma_{rad} > 0$) requires sub-horizon degrees of freedom with **exponential density of states** $\rho \propto e^{S(E)}$ and thermalization rates $\gamma_{int} \gg \omega$ --- i.e., black holes.
...
- $\sigma_T/T \approx 6.7\%$ (best-case *with* an SKA 21cm detection --- see the SKA-weighting caveat below; the DESI tomographic phase mapping alone already secures this same $6.7\%$)
...
**Caveat on the SKA weighting (June 2026 --- post-downgrade).** This 5-probe forecast adopts $\sigma_{21cm} = 1$ mK, i.e. a *clean* SKA 21cm detection. But the reionization-epoch oscillation amplitude is **theory-unpinned** (the reionization epoch falls within the first oscillation cycle after QCD ignition --- see the "Definitive Future Test" section and [Predictions §4](/predictions/)), so the SKA 21cm signal may be $\ll 1$ mK and possibly undetectable. The SKA contribution above (tr$(F) = 225$, and the "$\sigma_T/T \approx 6.7\%$ by SKA alone" attribution) is therefore a **best case, not a guaranteed constraint.** Crucially, the period $T$ does *not* hinge on SKA: the current 3-probe (DESI + Planck + DES) Fisher already constrains $\sigma_T/T \approx 6.7\%$ through the **DESI tomographic phase mapping alone** (the 4 LRG/ELG bins; see [Predictions §Fisher](/predictions/)) --- a robust, SKA-independent handle. SKA, if detected, would tighten the $\tau_0$--$L$ sector; absent a detection, $T$ is still pinned by DESI. (This reconciles the apparent attribution mismatch between this 5-probe forecast and the current 3x3 Fisher in [Predictions](/predictions/): both give $\sigma_T/T \approx 6.7\%$, but the robust source is DESI, not SKA.)
...
- **Tier 1 --- exact derivations (2)**: (i) the emergent-MOND geometric derivations --- $\mu(x) = x/\sqrt{1+x^2}$ from the Gauss-Codazzi quadrature (verified) and $a_0 = cH_0/2\pi$ (prior art: Milgrom/Verlinde) --- for galaxy rotation curves (SPARC RMS 29.3 km/s, $\mu(x)$ form rigorously derived); (ii) the DESI $w(z)$ mechanism *as method* --- the exact BDF ODE integration and the chronologically anchored stick-slip template ($\Delta$BIC $= -6.4$ on DR2) --- retained in T1 for the exactness of the calculation, its *amplitude/precision* claims carrying T2-level caveats (tier ruling July 2026; the Discoveries §8 table is the register of record). NOTE (audit May 2026): the $S_8$ and wide-binary (Chae 2025) entries previously listed here are DEMOTED --- $S_8$ to T2 (waveform/amplitude-dependent, growth sign a free bulk BC), wide binaries to T2 (the anomaly is observationally CONTESTED --- Pittordis-Sutherland/Banik find Newtonian --- and the $\gamma_g(u)$ boost is MOND's, not OBT-distinctive)
...
**3. The Overdetermined Jacobian (with one honest caveat).** In Extended $\Lambda$CDM, dark sector parameters are phenomenologically decoupled: $w_a$ tunes expansion, $\Sigma m_\nu$ tunes growth, $a_0$ tunes galaxies (independent epicycles). In OBT V8.2, the same base parameters ($f_{osc}$, $D$) that dictate the temporal harmonics of $w(z)$ also enter the structure growth ODE; the Jacobian of the parameter-to-observable map is **overdetermined** (4 parameters control $> 10$ observables). *Honest caveat:* the $w(z)$--$S_8$ coupling is not as rigid as a naive reading suggests --- $w(z)$ is set by the scalar channel (amplitude $A_w$) while $S_8$ is set by the tensor channel ($G_{eff}$ waveform), so the non-derivable slip-waveform shape can shift $S_8$ at the factor-$\sim$2 level without materially changing $w(z)$. The two are linked in *origin*, not rigidly *locked* in value. OBT nonetheless bridges galactic dynamics and cosmic expansion within a single 5D geometric framework, reducing (not eliminating) the need for scale-specific epicycles.
...
2. **Damping hierarchy**: Higher modes ($\ell$ >= 2) experience stronger dissipation (Q$_{1}$ < 4 vs Q$_{0}$ > 200)
...
with central mass M_c ~ 10$^{-12}$ MSun and width sigma_M approximately 1.5, spanning 10$^{-14}$ to 10$^{-10}$ MSun. The corresponding Schwarzschild radii:
...
**1. Wave-optics diffraction (fatal):** For M ~ 10$^{-12}$ MSun, the Schwarzschild radius is r_s approximately 3 nm. Subaru-HSC observes in the optical r-band (lambda approximately 600 nm). Since r_s << lambda, geometrical optics breaks down completely. The Fresnel-Kirchhoff diffraction parameter w_F = 2pir_s/lambda approximately 0.03 << 1 places these objects in the deep wave-optics regime where light diffracts around the PBH. The characteristic microlensing amplification pattern is entirely washed out. The telescope is physically blind.
...
**Below $M_{crit}$ (the capillaries):** The PBH is a **5D localized (Schwarzschild-Tangherlini) object** --- the stable, entropically favored end-state of the Gregory-Laflamme instability (the instability is of the uniform black *string*; the localized 5D hole is its product, not a transient undergoing it). It loses its **local 4D gravitational singularity** --- not its mass, but its concentrated on-brane $1/r$ cusp at sub-micron scales ($r \lesssim L$); its influence is diffused through the bulk Weyl tensor $E_{\mu\nu}$ via the Shiromizu-Maeda-Sasaki equations. These are the topological capillaries and quantum metronome nodes.

*Accretion-darkness audit (May 2026).* The absence of an accretion disk is **not** caused by this sub-micron geometry, and the earlier "softened potential suppresses Bondi-Hoyle accretion" reasoning was wrong on scale. Bondi-Hoyle accretion is set at the accretion radius $r_{acc} = 2GM/(v^2 + c_s^2) \sim 0.3$--$100$ m --- six to nine orders of magnitude **larger** than $L = 0.2\,\mu$m --- where the potential is fully 4D regardless of any modification at $r \lesssim L$ (indeed an exact 5D Tangherlini potential goes as $-1/r^2$, *steeper* near the horizon, not softer). The real reason these PBH are X-ray dark is mundane: an asteroid-mass object has a Bondi luminosity $L_{acc} = \eta\dot M c^2 \sim 10^{-28}\,L_\odot$, and the flow is collisionless (Knudsen number $\gg 1$), suppressing it further. The X-ray silence is **standard 4D astrophysics** (tiny mass, fast motion), shared by any asteroid-mass PBH --- not a 5D mechanism. The perforation hierarchy still governs the *microlensing* transition (Pillar B), but it does **not** gate accretion.

**Above $M_{crit}$ (brane-anchored):** The PBH's horizon extends beyond $L$, anchoring it firmly on the brane. It retains standard 4D gravity ($1/r$ potential), can form accretion disks, and participates in normal astrophysics.

*Heavy-seed caveat (May 2026).* The earlier claim that PBH just above $M_{crit}$ provide $10^3$--$10^5\,M_\odot$ "heavy seeds" for early SMBHs does **not** survive audit, on two counts: (i) Bondi accretion cannot grow them --- for an initial mass $\ll 10\,M_\odot$ the fractional growth over a Hubble time is bounded to $\sim 1.4\times$ (the growth integral scales linearly with the initial mass); and (ii) $10^3$--$10^5\,M_\odot$ lies $13$--$18$ orders **above** the stated EMF ceiling ($10^{-10}\,M_\odot$), so no such tail exists. OBT's early-SMBH explanation must therefore rest on the **capillary gas-channeling** mechanism (the $\sim 10^{20}$ kg anchors seed rapid baryonic agglomeration around topological nodes; see [Discoveries §5](/discoveries/)), **not** on PBH grown to SMBH mass by accretion.

**The ab initio derivation.** The EMF's operational window (10$^{-14}$ to 10$^{-10}$ MSun) is bounded on both sides by physics: below ~10$^{-14}$ MSun, Hawking evaporation destroys the PBH; above $M_{crit} \approx 6.8 \times 10^{-11} M_\odot$, the PBH becomes brane-anchored and visible. The capillary window is therefore set entirely by $L$ and fundamental constants --- no fine-tuning.
...
*Figure: Gregory-Laflamme perforation hierarchy. PBHs below $M_{crit}$ (purple) are 5D localized objects --- topological capillaries invisible to optical microlensing (and X-ray dark by standard low-Bondi accretion). PBHs above $M_{crit}$ (orange) remain brane-anchored with standard 4D gravity.*

### Epistemological Defense: Falsifiability of the Discrete Anchor Network

The convergence of three local immunities --- diffraction-blind in optical ($w_F \ll 1$), X-ray dark (the **standard** consequence of low-Bondi accretion at asteroid mass, $L_{acc} \sim 10^{-28}\,L_\odot$ --- **not** a 5D mechanism; see the accretion-darkness audit above), and Hawking-cold in gamma-ray ($T_H \approx 900$ K) --- poses a legitimate Popperian challenge. The honest answer (audit, May 2026): the network has six candidate macroscopic signatures, but they are **not** of uniform strength --- one is genuinely distinctive, two are consistency requirements shared with $\Lambda$CDM, and three are suppressed below observability or contested at OBT's own parameters ($f_{PBH} = 0.01$, asteroid mass):

- **(A) Ballistic Decoupling** --- $\sim 150$ kpc Bullet-Cluster offset. *Consistency requirement (shared with $\Lambda$CDM; falsifies pure MOND, not OBT-vs-$\Lambda$CDM).*
...
- **(D) Dynamical Heating** --- Poisson granularity in UFDs / wide binaries. *Empty at asteroid mass --- $\sim 10^{13}$ below threshold; predicts a null identical to $\Lambda$CDM.*
...
| Attractor Locking | 10<sup>-5</sup> s $\to$ ~1 Gyr | Transient | $\xi R\phi$ attractor locks period to T = 2.000 Gyr (chronodynamic eigenvalue) within ~2 e-foldings |
...
where $\delta\tau/\tau_0$ represents the **local tension contrast** (not the global bulk drift --- see below), estimated at about $2\times 10^{-4}$ in the Local Supercluster vicinity. A future program capable of measuring $H_0$ directionally at 0.05% precision over 10° patches could reveal this cosmic tension map --- regions where the membrane is tighter expand slightly faster.

Note: this $\sim 10^{-4}$ signal is **distinct from** the global $\sim 10^{-3}$ dipole generated by the brane's kinematic drift through the $AdS_5$ bulk (the Dark Flow / cosmic dipole mechanism --- see [Predictions](/predictions/) and [Discoveries §4.5](/discoveries/)). Both observables coexist: the bulk drift produces a single coherent dipole across the entire observable universe ($v_{bulk} \approx 300$ km/s), while the tension-map signal is a finer-grained patchwork reflecting local Cosmic Web structure.
...
2024     DESI detects dark energy evolution (4sigma)
   |
2025     S$_{8}$ tension addressed (time-dependent suppression ~4--10%, shear-low side)
...
2026    ⏳ CMB low-$\ell$ ISW (consistency check, ~1sigma)
   ...
   |
2027    DESI full survey $\to$ anchored w(z) template test (DeltaBIC)
   |    SKA-Low $\to$ 21cm reionization-history modulation (plausible, amplitude-uncertain)
   |
2030    CMB-S4 $\to$ ISW consistency check (~1sigma ceiling, not definitive)
...
- **S$_{8}$ tension**: Time-dependent growth suppression via oscillating G_eff(t), aligned with the cosmic-shear (DES/KiDS) side of the bifurcated tension

**⏳ Imminent Tests:**
- **Euclid 2025+**: sharpens w(z); combined with DESI Year 5 the anchored stick-slip template forecast reaches Decisive (DeltaBIC approximately −22) --- a template-level discrimination, not a per-bin 5sigma amplitude detection
...
**Detection**: template-level --- the chronologically anchored stick-slip w(z) (k = 1) is preferred over CPL at DeltaBIC = −6.4 on DESI DR2 *today*, with the Year-5 forecast DeltaBIC approximately −22 (Decisive). The amplitude A_w = 0.003 itself is far below any per-bin 5sigma detection; the discriminating power lives in the locked template.

### ISW Modulation in the CMB (a ~1sigma consistency check)
...
**Detection Method**: Our 2 Gyr membrane oscillation (1.6 x 10$^{-17}$ Hz) manifests through:
- **ISW effect**: a weak modulation of the CMB large-scale anisotropies at $\ell$ = 10-20 (~1sigma, cosmic-variance-capped; shown above)
...
The oscillation's ISW imprint is *consistent with* the (deficit-trending) Planck low-$\ell$ sky at the ~1sigma level --- a consistency check, not a detected pattern. DESI and Euclid probe the corresponding expansion-history template.

### Structure Growth Suppression (Time-Dependent Gravitational Oscillation)


*Figure 4: Time-dependent growth suppression. Late-Universe structures (DES, z < 0.5) grew during the current weakened-gravity phase (~4--10% slower, waveform-dependent); CMB/KiDS extrapolations from earlier phases remain quasi-standard.*
...
The current stretched phase ($G_\text{eff} < G_N$) produces a growth suppression of order 4--10% at low redshift (S$_{8}$ approximately 0.79, consistent with the cosmic-shear DES S$_{8}$ tension), while CMB-epoch gravity was exactly Newtonian (conformal protection). The precise value is waveform-shape dependent (see [Theory](/theory/)), not a +/-0.002 figure. Note the S$_{8}$ tension is bifurcated (shear-low vs cluster-high); OBT's suppression aligns with the shear side, and the eROSITA cluster-$\gamma$ link is exploratory only (audit May 2026).


*Figure 5: Structure growth suppression in oscillating brane model vs $\Lambda$CDM*


### SKA 21cm Reionization Modulation (a plausible order-of-magnitude future test)
...
Spatial tension variations create directional H$_{0}$ differences:
...
This 2 Gyr oscillation is far too slow for direct gravitational wave detection. Instead, a weak (~1sigma, cosmic-variance-limited) imprint appears in the CMB large-scale anisotropies through the ISW effect (§2).
...
|------|------------|-----------|---------|
| Newton @ 25 mum | No deviation | $L = 0.2\,\mu$m | Invisible |
...
|---------|-----------------|---------------------|
| Euclid + DESI Y5 | anchored stick-slip $w(z)$ template ($k=1$) vs CPL | forecast DeltaBIC approximately −22 in OBT's favor; an opposite-sign verdict refutes the anchoring |
| CMB-S4 | ISW consistency check (~1sigma ceiling) | --- (not a falsification channel; audit May 2026) |
...
**(b) ISW modulation** (Planck, compressed likelihood --- the *pre-audit* weight; post-audit the realistic ISW significance is ~1sigma (§2), and the headline $T$ constraint below is DESI-dominated, so it stands):
...
A Monte-Carlo evaluation of $E_G$ (`scripts/penrose_diosi_5d.py`, injection-tested: $\eta \to 1$ for $R \gg L$ and $\tau_{4D}$ matches Penrose's canonical estimate) with two bracketing crossover kernels (a sharp 4D$\to$5D match, and a resummed Randall-Sundrum form reproducing the Garriga-Tanaka $(2/3)(L/r)^2$ correction at $r \gg L$ and a $1/r^2$ 5D tail at $r \ll L$) gives a robust **enhancement of order unity**, growing as the object shrinks below $L$:
...
with $a_0 = cH_0/2\pi \approx 1.1 \times 10^{-10}$ m/s$^{2}$.
...
| Criterion | Oscillating Brane | $\Lambda$CDM | MOND |
|-----------|-------------------|------|------|
...
| **Free Parameters** | 4+1 ($\tau_0$, L, D, f_osc + N=6) | 2+ (Omega_c, sigma_v, m_chi) | 1 (a$_{0}$) + relativistic ext. |
| **CMB Fit Quality** | Analytically preserved (conformal freeze); CLASS/CAMB integration pending | $\chi^2$/dof approximately 1.00 | Poor without 2eV neutrinos |
...
| **S$_{8}$ Tension** | Consistent (~4--10% suppression, S$_{8}$approximately0.79, waveform-dependent) | 3sigma tension | Not addressed |
...
| S$_{8}$ suppression | ~4--10% (S$_{8}$approximately0.79, waveform-dependent) | +/-0.5% | Weak lensing | Current |
| w(z) amplitude A_w | 0.003 | +/-0.001 | BAO + SNe | 2025+ |
| H$_{0}$ anisotropy | 0.01% | +/-0.005% | Precision cosmology | 2030+ |
| **Gravitational Signatures** |
| Fundamental f$_{0}$ | 1.6 x 10$^{-17}$ Hz | +/-10% | ISW in CMB (weak, ~1sigma consistency level) | 2030+ |
| Oscillation period | 2.000 Gyr (eigenvalue) | +/-0.003 Gyr | Large-scale structure | 2028+ |
| ISW amplitude | ~10$^{-5}$ DeltaT/T | Factor of 2 | CMB-S4 (consistency check, cosmic-variance-limited) | 2030+ |
| **Galactic Scale** |
| MOND a$_{0}$ | 1.1 x 10$^{-10}$ m/s$^{2}$ | +/-5% | Galaxy dynamics | Current |
...
2. **Oscillation Signatures**:
   ...
   - A weak (~1sigma, cosmic-variance-limited) ISW-level imprint in CMB large-scale anisotropies --- a
...
3. **Modified Halo Structure**:
   - Fewer subhalos than $\Lambda$CDM (factor ~2-3)
...
4. **Spatial Gravity Variations**:
   ...
   - Directional H$_{0}$ variations ~0.01%
   - Future precision astrometry tests

5. **Baryon-DM Coupling**:
   - Tighter correlation than $\Lambda$CDM expects
...
- A clean failure of the anchored stick-slip $w(z)$ template against DESI Year-5 BAO (the forecast is
  DeltaBIC approximately −22 in the model's favor; the opposite verdict would refute the chronodynamic anchoring)
...
- Variations in fundamental constants beyond $|\dot{G}/G| > 10^{-13}$ yr$^{-1}$ at Solar System scales

*(The low-$\ell$ ISW signature, once listed here, is **not** a falsification criterion: it is
cosmic-variance-limited to a ~1sigma consistency check --- see the ISW audit note in
...
- Tension: $1$ J/m$^{2}$ = $2.43 \times 10^{-22}$ GeV$^{3}$
...
The Casimir energy density between two branes separated by distance $L$ in AdS$_{5}$:
...
**Issue**: Growth suppression of order 4--10% (BKM-derived phase; value waveform-shape dependent, S$_{8}$approximately0.79).
...
**4. Dimensional Reduction**: M2-brane wraps 2-cycle $\to$ effective 3-brane in 5D
...
For our fiducial parameters (M_PBH = 10$^{-11}$ M_⊙, f_PBH = 1%):
...
1. f_PBH < 0.1 for M ~ 10$^{-11}$ M_⊙ (from tau < 0.066)
...
- Serpico et al. (2020): Spectral distortions limit f_PBH < 0.1 for M ~ 10$^{-11}$ M_⊙
