...
*This-work debunk (the bridge).* The split is the **2-halo (environment)** term, not a $\mu(x)$ failure. Weak lensing uniquely probes the large-radius, low-acceleration regime where the *total* measured acceleration is $g_{\rm obs} = g_{\rm 1halo}[\mu(x)\text{ on baryons}] + g_{\rm 2halo}$, with $g_{\rm 2halo} = 4G\,b\,\Delta\Sigma_{\rm mm}(R)$ scaling with the galaxy **clustering bias** $b$. Early-type galaxies are more clustered (higher $b$) than late-types, so their lensing RAR rises more at low $g_{\rm bar}$. A direct computation (colossus matter correlation function at $z = 0.25$) gives a split in the **right direction** (early $>$ late) that **grows toward low acceleration** (where Brouwer's signal peaks) --- qualitatively as observed. **Quantitative honesty (hardening, this work):** with a *realistic* clustering bias derived from halo masses (colossus/Tinker-2010: early-type group-scale halo $\sim 5\times10^{12}\,M_\odot \Rightarrow b \approx 1.1$, late-type isolated central $\sim 6\times10^{11}\,M_\odot \Rightarrow b \approx 0.8$), the 2-halo split reaches only $\sim 0.07$--$0.11$ dex at the lowest accelerations --- about **one third** of Brouwer's $\sim 0.2$--$0.3$ dex. Reaching the full magnitude would require the early-type sample's **satellite/group** contribution (an effective bias $\sim 1.8$ pushes the split to $\sim 0.19$ dex, but only at the extreme low-$g$ end) **and/or** the baryonic-content branch (early vs late $M/L$ and gas at fixed stellar mass). The one-halo $\mu(x)$ law itself stays **universal**.

*Honest scope (downgraded after hardening).* This is therefore a **partial, plausible** environmental debunk, **not** a clean full reproduction: the *direction* and low-$g$ *growth* of the morphology split are robustly environmental (not a $\mu(x)$ failure), but with realistic central bias the 2-halo accounts for only $\sim 1/3$ of the amplitude --- the remainder rests on satellite/group effective bias and baryonic content that are plausible but **not established here**. An order-of-magnitude model (comoving $\Delta\Sigma$, point-mass one-halo); a per-bin fit to Brouwer's split and a second lensing survey (DES/HSC) are needed to settle the full amplitude. The boost is **MOND-shared**. It is a debunk of an *external* claim ("the lensing-RAR morphology split falsifies modified gravity"), **not** a modification of OBT. Its main value is as a **bridge**: it extends the same $a_0/\mu(x)$ from kinematics into the **lensing** (photon) and **cosmic-environment** (2-halo / clustering) domains. *Tier: T2 (partial --- direction solid, full amplitude open).*
