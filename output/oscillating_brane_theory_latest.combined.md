...
$$\boxed{S_8^{OBT} = 0.836 \times 0.955 \approx 0.80 \pm 0.01\;\text{(waveform parametrization)}}$$

**Waveform parametrization uncertainty.** The exact S$_{8}$ value depends on two EFT choices internal to the stick-slip waveform: the slip steepness parameter ($k_{slip} \in [3, 5]$) and the numerical centering of the waveform mean. Independent implementations using different parametrizations yield $S_8 \in [0.796, 0.798]$ with exact mean centering, and $S_8 \in [0.789, 0.790]$ with approximate centering. The full envelope is $S_8 \approx 0.80 \pm 0.01$, comfortably within the DES Year 6 band ($0.790 \pm 0.018$). The exact waveform profile (and hence the precise $S_8$ digit) will be determined by the 5D NR simulation; within the current EFT, it is an irreducible $\sim 1\%$ parametrization uncertainty.
