...
### Card #22 --- The cluster two-scale anatomy is OBT's derived structure: sinc extinction + Weyl core (the first dynamical OBT-vs-MOND card)

**External theory debunked (within OBT, true if OBT true):** Eckert et al. (2022, X-COP gravitational field): "no universal acceleration law works in galaxy clusters --- the cluster RAR strongly departs from the galaxy RAR, MOND is rejected at high significance, and an *ad-hoc* two-acceleration-scale form ($g_1, g_2, \alpha_1, \alpha_2$, four fitted parameters) is required." **The card:** the two "mystery scales" are OBT's *own derived structure*. In clusters $t_{dyn}(r)$ sweeps 0.25 $\to$ 5.6 Gyr from the BCG to $r_{500}$, so the genuine $\mu(x)$ boost dies center-to-edge as $W(r) = \vert\mathrm{sinc}(\pi t_{dyn}(r)/2\,\mathrm{Gyr})\vert$ --- *zero freedom*, the V8.2 sinc theorem run radially: that is $g_1 = a_0 W(t_{dyn}(r))$; the second scale is the cored self-similar Weyl component (the geometric dark matter), two global parameters in all ($f_W = 0.70$, $r_c = 0.043\,R_{500} \approx 55$ kpc). **The verdict** (probe `xcop_hier`; X-COP per-bin product files, 12 clusters, 75 bins, BCG zone included; the cards-#6/#21 hierarchical machinery with $\sigma_{int} = 0.327$ *calibrated on the ad-hoc model's own best case* --- conservative against OBT): the derived 2-parameter structure fits at $\chi^2/N = 1.04$ and **beats the 4-parameter ad-hoc class at $\Delta$BIC $= +5.5$**, with flat residuals ($-0.06/-0.04$, sub-$\sigma_{int}$), while pure MOND --- full boost at all radii --- is excluded at $\Delta$BIC $= +648$ ($\chi^2/N = 9.8$; and 433 under measurement errors alone). **OBT-distinctive:** the sinc extinction is OBT's own mechanism, *not* MOND-shared --- this is the game's first dynamical OBT-vs-MOND card, won precisely on the terrain where MOND historically dies, and it simultaneously explains *why* MOND fails there (the boost is extinguished, not merely insufficient) and *why* an ad-hoc two-scale form seemed necessary (two real scales exist --- one derived, one the Weyl core). The earlier phases are on the record: the in-house recon $R(r_{500}) = 1.53 \pm 0.09$, the three-regime anatomy mapping, and the round-3 refusal under the naive error model (the rule held until the variance treatment was proper). **Honest scope:** $\sigma_{int} = 0.33$ is hydrostatic-bias-scale (plausible against the literature's 20--40% mass biases, but absorbed, not derived); the compact Weyl core ($\sim$55 kpc) carries the classic central-mass concentration --- flagged, as it bears on the cored-Weyl narrative at cluster scales; BCG modeled crudely (universal $1.2\times10^{12}\,M_\odot$ Hernquist); the simple $\vert\mathrm{sinc}\vert$ form (the V8.2 multi-harmonic version would shift details); bin thinning approximates independence; 12 of 13 clusters (Hydra A absent from the package); $\Delta$BIC $+5.5$ is positive-to-strong, not decisive --- the first cluster card, not the last word.
...
### Card #29 --- Malin 1: the boost deficit is organized by internal period, not by acceleration

**External theory debunked (within OBT, true if OBT true):** "Constant-$a_0$ MOND over-predicts Malin 1's ultra-extended outer rotation --- Lelli, Fraternali & Sancisi 2010 published a $\sim$25 km/s (12--13%) over-prediction requiring a negative outer $M/L$ or a 6° outer-warp escape ($i: 38° \to 32°$); the giant LSB *'creates problems for MOND'*." **The card (probe `malin1_ara`; their own baryonic decomposition EPS-extracted and validated to 0.1 km/s against the published table):** at *standard* $a_0$ (they used the low Bottema value), the constant-law misfit worsens to $+3.3\sigma/+2.5\sigma$ at the two outer points ($i \pm 3°$ systematic folded) --- **and the failure is exactly localized in the ARA resonance band**: the two failing points sit at $T_\kappa = 1.38/1.78$ Gyr (in-band) while the two inner points ($T_\kappa = 0.23/0.75$, sub-band) are perfect ($-0.2/+0.3\sigma$). The same-paper control NGC 7589 is entirely sub-band ($T_\kappa = 0.13$--0.90 Gyr): ARA predicts its good constant-MOND fit unchanged --- preserved. Applying the band envelope **measured independently in the SPARC stack before this hunt** ($\mathcal{W}_{emp} = 0.80/0.71$, 95% bound 0.61 --- zero per-object freedom, zero glue): residuals drop to $+2.2/+1.2\sigma$ (central envelope) and $+1.0/+0.7\sigma$ (envelope edge); the published warp escape shrinks to at most half ($i: 38° \to 35.5°$). **The why** = the boost is not a constant of nature but the adiabatic response to the oscillating tilt: tracers whose epicyclic period enters the brane-period resonance band average part of the boost away. Constant-$a_0$ MOND has **no axis** that can localize failure by *internal period at fixed acceleration*; OBT does --- and the deficit sits on OBT's pre-measured envelope, with the same $T_\kappa$-organized deficit appearing independently in the SPARC outermost-point stack ($3.4\sigma$, ~30 points, ~20 galaxies). Third OBT-vs-MOND dynamical discriminant (after the cluster anatomy #22 and the brane drift #23), the first ARA-based one. **Honest scope:** a defensive-plus-organizational card --- *"Malin 1 does not falsify the boost law, and its failure pattern is OBT's structure"* --- not *"OBT fits perfectly"*: at the envelope's central values a $\sim 2\sigma$ residual remains (absorbed by half the granted warp or the envelope's lower range); the SPARC envelope attribution is EFE-degenerate (named separator: the $e$-vs-$T_\kappa$ cross-regression); the band envelope is the EFT-empirical part of ARA, not the derived part; Malin 1's rotation curve is four points of sparse VLA data (Lelli+10's own caution carried); one band-crossing galaxy so far --- the MHONGOOSE/WALLABY ultra-extended discs are the replication path, with UGC 9133/NGC 289/UGC 128 the named next targets. **Replication update (June 2026, probe `band_trio`):** the named targets delivered from the SPARC cache itself --- **6/6 band-crossing galaxies** (UGC 9133, NGC 289, UGC 128, UGC 1230, NGC 3769, NGC 5055) show their in-band points below their own sub-band points (median internal split $\Delta = -0.103$ dex; sign test $p = 1/64$), a per-galaxy statistic in which mass-to-light, distance and mean inclination *cancel* (only radius-dependent warps survive, and they would need to conspire six times in the same direction); the per-object EFE budgets ($e_{env} = 0.024$--$0.04 \to \leq 0.04$ dex) cannot pay the observed deltas. Malin 1 is no longer alone: seven band-crossers now carry the pattern.

### Card #30 --- The outer decline has a second axis: internal period, at fixed environment

**External theory debunked (within OBT, true if OBT true):** "The outer-radius downturn of the radial acceleration relation is the External Field Effect --- an *environmental* axis only (the Chae 2020/2021 reading; the AQUAL interpretation of declining outer curves). Constant-$a_0$+EFE admits no internal-period dependence at fixed environment." **The card (probe `band_separator`; 124 SPARC galaxies with Chae's own erratum-table $e_{env}$):** partial Spearman correlations on outermost-point RAR residuals: the **internal-period axis survives at fixed environment** --- resid $\sim T_\kappa \vert e$: $r = -0.261$, $p = 0.0015$ --- while the environmental axis is weak in this regime (resid $\sim e \vert T_\kappa$: $r = -0.097$, $p = 0.14$), and the two axes are **mutually uncorrelated** ($-0.087$: no collinearity escape). The low-environment half of the band galaxies keeps its deficit ($-0.044 \pm 0.026$ --- under EFE-only it should sit at zero); the sub-band control is clean ($+0.007$); and the isolated band-crosser Malin 1 (card #29) carries the largest deficit ($0.13$--$0.15$ dex) with **no environmental budget for it** ($e \sim 0.03$ would give $\leq 0.03$ dex). **The why** = ARA: the outer boost deficit has two physical sources with different axes --- *where the galaxy lives* (the EFE, card #16) and *how slowly its orbits turn* (the resonance-band averaging of the oscillating tilt). Constant-$a_0$ frameworks, with or without the EFE, possess only the first; no such framework can organize residuals by internal period at fixed acceleration and fixed environment. **Retro-effect:** discharges the EFE-degeneracy caveat carried by card #29 and by the band-envelope measurement. **Honest scope:** the EFE itself is *not* refuted --- card #16 stands (Chae's full-curve hierarchical detection is a finer instrument than outermost-point medians; both effects are real, this card ranks their outer-point projections); $e_{env}$ is crude and covers 124/175 galaxies; the band split is small-N (6/7); the $e$-axis weakness is regime-specific, not global. **Not MOND-shared**: this is the fourth OBT-distinctive discriminant family ($a_0(z)$, the cluster anatomy #22, the brane drift #23, and now the ARA pair #29+#30).

### Card #31 --- Tidal dwarf galaxies: the test that assumed what its own table refutes

**External theory debunked (within OBT, true if OBT true):** "TDGs are a decisive test of MOND-family laws, and they fail it: six bona-fide tidal dwarfs under-rotate against MOND+EFE by 27--86% (Lelli et al. 2015 --- *'a challenging test for alternative theories like MOND'*; joint $3.6\sigma$ assuming equilibrium)." **The card (probe `tdg_books`; both Lelli+15 tables verbatim):** the test's own kinematic table voids its premise --- $t_{merg}/t_{orb} = 0.2$--$0.8$ for **all six** objects: none has completed a single orbit since formation, a disc that has executed a fifth of a turn is not an equilibrium rotator, and gas placed on new orbits *under-rotates until it circularizes* --- exactly the observed direction; the authors' own concession is verbatim (*"it remains unclear whether this would still hold in MOND"*). **Second layer (the ARA bonus):** their tiny rotation velocities make **5/6 TDGs band objects** ($T_\kappa = 0.49$--$2.12$ Gyr) --- even at equilibrium the boost law predicts less than constant-MOND there; the pre-measured SPARC envelope shaves the joint discrepancy from $3.6$ to $3.1\sigma$ with zero per-object freedom, and the worst object (NGC 7252NW) is simultaneously the least-orbited *and* the deepest in the band. The deficit-versus-$t_{orb}$ organization ($+0.60$, $p = 0.21$ at $n = 6$) is the suggestive fingerprint both effects share. **Settles the registry's logged tension** (the old "~30--40%, plausibly non-equilibrium" note is superseded: the audited spread is 27--86%, the premise is void by the test's own numbers, and the band accounts for a further measured share). **Future discriminant named:** *old* tidal dwarfs (several orbits completed) in the band --- ARA predicts their equilibrium deficit persists (~5--15% in $V$ per the envelope); pure non-equilibrium predicts it vanishes: a clean OBT-vs-MOND test when deep samples arrive. **Honest scope:** a dissolution card (premise removal), not a positive fit --- assuming equilibrium, a $3.1\sigma$ residual would remain after ARA; $n = 6$ and the two effects are degenerate on this sample (both axes are $t_{orb}$); MOND-shared dissolution (Kroupa 2012 and Ploeckinger-class non-equilibrium arguments have priority --- our additions are the verbatim orbit-count audit across all six, the band identification, the envelope-corrected ledger, and the old-TDG discriminant).
