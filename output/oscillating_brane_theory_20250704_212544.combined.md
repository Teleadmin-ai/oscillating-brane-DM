...
$$E_{tens} = \frac{1}{2} $\tau$_0 A \left(\frac{2$\pi$z}{$\lambda$}\right)^2$$
...
Beyond stellar and supermassive black holes, a hidden population could play a crucial role: primordial black holes (PBH). A PBH of mass 10$^-$$^1$$^1$ M_☉ has a Schwarzschild radius r_s $\approx$ 30 nm, creating a funnel comparable in size to our extra dimension L.
...
D$_+$^osc/D$_+$^ΛCDM(z=0) = 0.948 (-5.2%)
...
|------|-------------|--------------|---------|
| Newton @ 25 $\mu$m | Aucune déviation | L = 0.2 $\mu$m | ✓ Invisible |
| PTA 15 ans | h_c < 3×10$^-$$^1$$^5$ | h_c ~ 2×10$^-$$^1$$^8$ | ✓ Silencieux |
| H$_0$ dipole | < 2% | ~0.01% | ✓ Subtle |
...
Computes linear growth factor D$_+$(z) including oscillation effects.
...
|------|------------|-----------|---------|
| Newton @ 25 $\mu$m | No deviation | L = 0.2 $\mu$m | ✓ Invisible |
| PTA 15 years | h_c < 3×10$^-$$^1$$^5$ | h_c ~ 2×10$^-$$^1$$^8$ | ✓ Silent |
| H$_0$ dipole | < 2% | 1.5% | ✓ Subtle |
...
$$Π(t) = \sum_i \dot{N}_i m_{MN} v_$\perp$ $\simeq$ f_{osc} $\rho$_{DM} v_$\perp$^2$$
...
$$k_{eff} = \frac{$\partial$^2E}{$\partial$z^2} = \frac{$\tau$_0 A}{R_H^2} $\approx$ $\tau$_0$$
...
$$$\omega$_$\ell$ $\simeq$ \sqrt{$\ell$($\ell$+1)} × $\omega$_0$$
...
$$T = 2$\pi$\sqrt{\frac{M_{osc}}{k_{eff}}} = 2$\pi$\sqrt{\frac{f_{osc} M_{DM,tot}}{$\tau$_0}}$$

Inverting for the observed period T = 2.0 Gyr:

$$$\tau$_0 = f_{osc} M_{DM,tot} \left(\frac{2$\pi$}{T}\right)^2 = 7.0 × 10^{19} \text{ J/m}^2$$
...
$$\frac{$\delta$H}{H} $\simeq$ \frac{1}{2} \frac{$\delta$$\tau$}{$\tau$_0} $\approx$ 10^{-4}$$
