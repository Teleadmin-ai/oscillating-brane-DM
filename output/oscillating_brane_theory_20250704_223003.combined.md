...
Where:
- $\tau$$_0$ = 7.0 × $10^{1}^{9}$ J/m² is the brane tension
...
With Kelvin-Voigt damping $\gamma$ ~ $10^{-2}$ Gyr⁻¹:
...
2. **Gravitational wave signature** at f$_0$ $\approx$ 1.6 × $10^{-17}$ Hz
...
|-------------------|------------------|------------------------|
| Brane tension | $\tau$$_0$ = 7.0 × $10^{1}^{9}$ J/m² | The elasticity of spatial fabric |
| Oscillation period | T = 2.0 ± 0.3 Gyr | The cosmic heartbeat |
| MOND acceleration | a$_0$ = 1.1 × $10^{-10}$ m/s² | Gravity at the edge |
...
| $\tau$$_0$ | 7.0 × $10^{1}^{9}$ J/m² | The tension maintaining space |
...
Using the conversion: 1 GeV³ = 3.24 × $10^{2}^{4}$ J/m²
...
For a membrane of radius R_H = c/H$_0$ = 1.33 × $10^{2}^{6}$ m (the Hubble horizon, the distance to which we can see), the deformation energy is:
...
$\tau$$_0$ = f_osc M_DM,tot (2$\pi$/T)² = 7.0 × $10^{1}^{9}$ J/m²
...
In this framework, the brane appears at the Big Bang with quasi-Planckian tension $\tau$_BB ~ $10^{5}^{0}$ J/m²—a membrane stretched to breaking point, vibrating with pure energy.
...
**Phase II - Brane Reheating ($10^{-3}^{4}$ - $10^{-3}^{2}$ s)**: Tension drops abruptly via massive production of dark matter/anti-dark matter pairs in the bulk. This "quantum evaporation" dissipates excess energy, leaving residual tension around $10^{3}^{0}$ J/m².
...
a$_0$ = (cH$_0$/2$\pi$) × $\xi$ = 1.1 × $10^{-10}$ m/s²
...
| GW doublet | h_c $\approx$ 2×$10^{-18}$ (detectable) | <$10^{-1}^{9}$ (likely undetectable) |
...
- **Main peak**: f$_0$ = 1/T $\approx$ 1.6 × $10^{-17}$ Hz
...
| PTA 15 ans | h_c < 3×$10^{-1}^{5}$ | h_c ~ 2×$10^{-18}$ | [check] Silencieux |
...
| **Direct Detection** | $\sigma$ < $10^{-4}^{8}$ cm² forever | $\sigma$ > $10^{-4}^{7}$ cm² expected | No prediction |
| **S$_8$ Tension** | Resolved (-5.2%) | 3$\sigma$ tension | Not addressed |
| **H$_0$ Tension** | Potential resolution | 5$\sigma$ tension | Not addressed |
| **GW Prediction** | f$_0$ = 1.6×$10^{-17}$ Hz | None specific | None |
...
| Brane tension $\tau$$_0$ | 7.0 × $10^{1}^{9}$ J/m² | ±15% | Indirect via H$_0$(z) | Current |
...
| **Gravitational Waves** |
| Fundamental f$_0$ | 1.6 × $10^{-17}$ Hz | ±10% | PTA arrays | 2035+ |
| Strain h_c | 2 × $10^{-18}$ | Factor of 3 | SKA-PTA | 2035+ |
| Spectral index n_t | 2/3 | ±0.1 | NANOGrav+ | 2025+ |
| **Galactic Scale** |
| MOND a$_0$ | 1.1 × $10^{-10}$ m/s² | ±5% | Galaxy dynamics | Current |
...
| DM cross-section | < $10^{-4}^{8}$ cm² | Lower limit | Direct detection | Current |
...
```
5D line element: ds² = -α²dt² + $\gamma$ᵢⱼ(dxⁱ + βⁱdt)(dxʲ + βʲdt) + $\phi$⁴dz²
...
- **SKA-PTA**: 
  ...
  - Search for f$_0$ = 1.6×$10^{-17}$ Hz doublet
...
|-------|-----------|--------------|------------------|--------|------------|
| Oscillating | $\tau$$_0$ | Log-uniform | [$10^{1}^{9}$, $10^{2}^{0}$] | J/m² | Scale-invariant prior for unknown energy scale |
...
|-----------|------|--------|-----|--------|-----|
| $\tau$$_0$ (J/m²) | 7.08×$10^{1}^{9}$ | 7.00×$10^{1}^{9}$ | 1.07×$10^{1}^{9}$ | [6.03×$10^{1}^{9}$, 8.13×$10^{1}^{9}$] | 1.000 |
...
| Relaxation | $10^{-3}^{2}$ s → 1 Gyr | $10^{2}^{7}$ → 7×$10^{1}^{9}$ | $\tau$ $\propto$ t⁻¹/², fundamental mode enters resonance $\approx$ 1 Gyr |
...
- Stable tension $\tau$$_0$ = 7×$10^{1}^{9}$ J/m²
...
- **Fundamental**: f$_0$ = 1.6 × $10^{-17}$ Hz
- **Echo**: 2f$_0$ from flux reversal at membrane extrema
- **Strain**: h_c ~ 2 × $10^{-18}$ at f$_0$, ~ $10^{-18}$ at 2f$_0$
...
| PTA 15 years | h_c < 3×$10^{-1}^{5}$ | h_c ~ 2×$10^{-18}$ | [check] Silent |
...
- **Primary peak**: f$_0$ = 1/T $\approx$ 1.6 × $10^{-17}$ Hz
- **Echo**: 2f$_0$ (reversal harmonic)
...
This return deposits a momentum "hit" $\delta$p ~ m_MN × v_$\perp$ radially opposite to the outgoing flux. The surface density of these impacts, summed over all black holes, creates a periodic pressure:
...
If these PBHs represent a fraction $\Omega$_PBH ~ $10^{-4}$ of cosmic density, they form a dense network of small-scale entry points. Like thousands of needles piercing fabric, they increase the oscillating fraction f_osc without changing the macroscopic dark matter density. 
...
| Brane tension | $\tau$$_0$ = 7.0 × $10^{1}^{9}$ J/m² | The elasticity of spatial fabric |
| Oscillation period | T = 2.0 ± 0.3 Gyr | The cosmic heartbeat |
| MOND acceleration | a$_0$ = 1.1 × $10^{-10}$ m/s² | Gravity at the confines |
...
Early versions imagined dark matter oscillating like a mass on a spring, with energy E $\propto$ z². This simplistic image led to absurdities: periods shorter than Planck time or stiffnesses exceeding any known physical scale.
...
For a membrane of radius R_H = c/H$_0$ = 1.33 × $10^{2}^{6}$ m (the Hubble horizon, how far we can see), the deformation energy is:

$$E_{tens} = \frac{1}{2} \tau_0 A \left(\frac{2\pi z}{\lambda}\right)^2$$

Where:
- $\tau$$_0$: membrane tension, like a drumhead's
- A $\simeq$ R_H²: vibrating membrane area (the entire observable universe!)
- z: displacement amplitude in the hidden dimension
- $\lambda$ $\simeq$ 2R_H: fundamental mode wavelength
...
The brane appears at the Big Bang with quasi-Planckian tension $\tau$_BB ~ $10^{5}^{0}$ J/m²—a membrane stretched to breaking point, vibrating with pure energy.

### Phase I - Trans-membrane Inflation (0 - $10^{-3}^{4}$ s)
...
### Phase II - Brane Reheating ($10^{-3}^{4}$ - $10^{-3}^{2}$ s)

Tension drops brutally via massive production of dark matter/anti-dark matter pairs in the bulk. This "quantum evaporation" dissipates excess energy, leaving residual tension around $10^{3}^{0}$ J/m².

### Phase III - Slow Stabilization ($10^{-3}^{2}$ s - 100 Myr)
...
where $\delta$$\tau$/$\tau$$_0$ represents the local tension contrast, estimated at ~2×$10^{-4}$ in the Local Supercluster vicinity. A future program capable of measuring H$_0$ directionally at 0.05% precision over 10° patches could reveal this cosmic tension map—regions where the membrane is tighter expand slightly faster!
