...
| MOND acceleration | a$_0$ = 1.1 $\times 10^{-10}$ m/s² | Gravity at the edge |
...
a$_0$ = (cH$_0$/2$\pi$) × $\xi$ = 1.1 $\times 10^{-10}$ m/s²
...
| GW doublet | h_c $\approx$ 2$\times 10^{-18}$ (detectable) | <$10^{-1}^{9}$ (likely undetectable) |
...
| PTA 15 ans | h_c < 3×$10^{-1}^{5}$ | h_c ~ 2$\times 10^{-18}$ | [check] Silencieux |
...
| **Galactic Scale** |
| MOND a$_0$ | 1.1 $\times 10^{-10}$ m/s² | ±5% | Galaxy dynamics | Current |
...
| PTA 15 years | h_c < 3×$10^{-1}^{5}$ | h_c ~ 2$\times 10^{-18}$ | [check] Silent |
...
- **Primary peak**: f$_0$ = 1/T $\approx$ 1.6 $\times 10^{-17}$ Hz
- **Echo**: 2f$_0$ (reversal harmonic)
...
| Brane tension | $\tau$$_0$ = 7.0 × $10^{1}^{9}$ J/m² | The elasticity of spatial fabric |
| Oscillation period | T = 2.0 ± 0.3 Gyr | The cosmic heartbeat |
| MOND acceleration | a$_0$ = 1.1 $\times 10^{-10}$ m/s² | Gravity at the confines |
...
where $\delta$$\tau$/$\tau$$_0$ represents the local tension contrast, estimated at ~2$\times 10^{-4}$ in the Local Supercluster vicinity. A future program capable of measuring H$_0$ directionally at 0.05% precision over 10° patches could reveal this cosmic tension map—regions where the membrane is tighter expand slightly faster!
