...
**1. Conformal Freeze-Out (Radiation Era):** During the BBN epoch, the universe is dominated by a relativistic plasma (photons, neutrinos, e+/- pairs) with $w_{eff}$ = 1/3. The trace vanishes rigorously:
...
| **Free Parameters** | 3 ($\tau_0$, $f_{osc}$, L) | 2+ (Omega_c, sigma_v, m_chi) | 1 (a$_{0}$) + relativistic ext. |
...
def log_prior(theta):
    ...
    tau_0, $f_{osc}$, T_osc = theta
    
# Theoretical constraint: $\tau_0$ = $f_{osc}$ * M_DM * (2pi/T)$^{2}$
    M_DM = 1e24  # kg (galaxy mass scale)
    tau_0_expected = $f_{osc}$ * M_DM * (2*np.pi/T_osc)**2
...
# Bounds on individual parameters
    if not (1e19 < tau_0 < 1e20):  # J/m$^{2}$
        return -np.inf
    if not (0.1 < $f_{osc}$ < 0.9):     # Fraction
        return -np.inf
...
| | $f_{osc}$ | Uniform | [0.05, 0.20] | - | Weak prior based on halo core constraints |
...
| $f_{osc}$ | 0.100 | 0.100 | 0.020 | [0.081, 0.120] | 1.000 |
...
brane = BraneOscillator(
    ...
    $f_{osc}$=0.10,    # Oscillating fraction
    T=2.0          # Period (Gyr)
)
...
class BraneOscillator:
    def __init__(self, tau_0=7.0e19, $f_{osc}$=0.10, T=2.0, L=2.0e-7):
        """
        Parameters:
        - tau_0: Brane tension (J/m$^{2}$)
        - $f_{osc}$: Oscillating DM fraction
        - T: Period (Gyr)
        - L: Extra dimension size (m)
        """
```
